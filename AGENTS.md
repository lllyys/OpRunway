...
- 私有主机名 / 真实远端路径 / token **一律走 `OPRUNWAY_*` 环境变量传入，绝不写进仓**（仓里的默认值都是占位）。
...
- **两条都是真机验收通路、都产验收裁决**（`run_workflow.py:37` `_REAL_MACHINE_MODES = {"new_example", "aclnn_py"}`）。别写成「`new_example` 是唯一产裁决的路」——median+PR6429 的真机 56/56 精度 PASS 正是 `aclnn_py` 跑出来的。
...
3. **零硬编码**：仓名 / 路径 / SOC / 目标算子 / 精度阈值统统不写死，运行时探测或直接问用户；产物只落用户 CWD 的 `reports/`，不碰 `~/.config`、不改 shell rc。
...
| **当前状态与 TODO / 交接** | `doc/oprunway-session-handoff-2026-07-25.md`、`doc/oprunway-todo.md`、`doc/oprunway-changes-brief.md`（改动流水） |
...
- **经真 NPU 验收裁决坐实的算子**：IsClose / Sign（A3）· **Median（A3，PR6429，精度 56/56 PASS）** · Elu / Silu（A5-950，18/18 非空例）。
- ⚠ **Median 的性能维仍 BLOCKED**（msprof 采集侧问题、**非 PR 缺陷**）：精度已出裁决，**性能一个有效耗时数都没产出**。不得表述成「性能已验证 / 已达标」，也别反过来说「从没上过真机」。
- **`aclnn_py` 的 perf 通路**：采集与比较代码已接通，但**至今没产出过有效耗时数**。**`covered ≠ 真机绿`。**
