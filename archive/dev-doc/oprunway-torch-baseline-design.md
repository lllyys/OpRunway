...
见证算子：median（PR6429，aclnn 两段式、双输出 values+indices）。落地环境：a3 专用容器（torch_npu 2.10 / CANN 9.0.1 / 单卡 davinci0 / 根盘 8.6G）。
...
~~**未上真机提醒**（承 golden-branch-handoff）：全部为静态集成点+设计，torch_allclose NPU 实测、ctypes-aclnn 在 9.0.1 运行时能否跑通、多输出 arity 解析、bf16 窄化、torch_npu perf kernel-only 口径——**均须 a3 专用容器真机验证，covered≠真机绿**。~~
...
在 a3 专用容器（CANN 9.0.1 / torch_npu 2.10）只读探测，结论如下——**凡与上文冲突以本节为准**。
...
### 9.6 D0/D1/D2 实测（2026-07-24 a3 专用容器坐实）
