...
- [ ] **⚠ follow-up · 真机 opp provenance 绑定（codex 门坐实、阻断 bf16 转 tested）**：现 opp 复用**不绑当前 op 源**（OPHASH 用 `experimental/math/$OP`、对 isclose 路径不存在=恒定空 hash；复用只查 `op_api/include` 目录在否）→ 无法自动排除 stale opp 假通过、复用为 **dev-grade**。**bf16 虽 a3 真 NPU 实测精度全过（含完整 3-dtype），但因证据未 provenance-clean（复用了 prior opp、未从当前源重建）→ 暂留 deferred、不转 tested**（守本项目「证据 provenance 绑定、防 stale 假通过」）。修法：正确 op 源路径算 OPHASH + opp 独立 provenance stamp（绑 op 源 hash/SOC/vendor/构建参数）+ 缺 provenance/路径不存在则 fail-closed + 从当前源重建；修后从源重建重验、再转 tested。
