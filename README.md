...
**NPU 算子「验收（acceptance）」workspace。** 把下面这条验收流水线做成可复用的 workflow / agents / skills：

```
任务书 + 被测源码 ──①用例生成(ST)──▶ 测试用例集 ──②NPU跑测──▶ NPU 精度裁决 + msprof 性能数据
```

- **Task 1 · 用例生成(ST)**：调用方给定的算子任务书 + 被测源码 → 覆盖功能/精度/性能的机读+人读用例集。
- **Task 2 · NPU 跑测**：用同一套用例，在算子工程上跑出 NPU 精度对比 + 性能数据。

Workflow 不负责也不操作 GPU：不连接、不运行、不采集、不消费 GPU 数据。任务书要求 GPU 精度真值时，
按仓规解析为同族 CPU；要求 GPU 性能对比时只测 NPU msprof，并如实记录未验收条款。
