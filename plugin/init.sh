...
选项:
  ...
  --force           冲突时覆盖，先备份 <file>.bak.<时间戳>
  --uninstall       逆操作：删本脚本装的 symlink、还原备份、移除 CLAUDE.md 托管块
...
# day-1 实跑面：只有 claude 真写；其余强制干跑（静态审）
REAL_WRITE=1
if [ "$TOOL" != "claude" ] && [ "$UNINSTALL" -eq 0 ]; then
  if [ "$DRY_RUN" -eq 0 ]; then
    warn "day-1 仅 Claude 分支实跑；'$TOOL' 为静态审/干跑（真机多 CLI 验证挂起，见 open_decision④）→ 自动转 --dry-run"
    DRY_RUN=1
  fi
fi
...
backup_file() {
  ...
  if [ "$DRY_RUN" -eq 1 ]; then plan "备份 $f -> $bak"; else cp -p "$f" "$bak"; log "已备份 $f -> $bak"; fi
}
...
_block_present() { [ -f "$1" ] && grep -qF "$MANAGED_BEGIN" "$1"; }

install_reg() {
  if _block_present "$REG_FILE"; then
    log "幂等跳过（托管块已在）: $REG_FILE"
    return 0
  fi
  if [ "$DRY_RUN" -eq 1 ]; then
...
# ── 安装 / 卸载主流程 ─────────────────────────────────────────────────────────
_iter_skills() { find "$PLUGIN_ROOT/skills" -mindepth 1 -maxdepth 1 -type d 2>/dev/null; }
_iter_agents() { find "$PLUGIN_ROOT/agents" -mindepth 1 -maxdepth 1 -name '*.md' 2>/dev/null; }
...
_rm_path() {   # 删一个我们装的 symlink/materialize 拷贝（仅删我们管的）
  local p="$1"
  if [ -L "$p" ]; then
    if [ "$DRY_RUN" -eq 1 ]; then plan "rm symlink $p"; else rm -f "$p"; log "删 symlink $p"; fi
  elif [ -e "$p" ]; then
    warn "跳过（非 symlink，疑非本脚本所装，保守不删）: $p"
  fi
}
...
$(_iter_agents)
EOF
  # 移除 CLAUDE.md/AGENTS.md 托管块
  if _block_present "$REG_FILE"; then
    if [ "$DRY_RUN" -eq 1 ]; then
      plan "从 $REG_FILE 移除 OpRunway 托管块（先备份）"
    else
      backup_file "$REG_FILE"
      # 删除 BEGIN..END 之间（含标记行），用 awk 保守处理
    ...
    log "无托管块可移除: $REG_FILE"
  fi
  log "卸载完成（备份 .bak.* 保留，需要可手工还原）。"
}
