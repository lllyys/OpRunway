---
name: op-acceptance
description: 跑一个 NPU 算子的验收流水线——输入=算子任务书(md 路径或链接)+PR 链接，自动产 spec→跑测→跑确定性脚本；总结工件按 acceptance_artifacts.formal_acceptance_allowed 在 formal / attempt 间二选一。
...
- 判定唯一归**确定性脚本链**（`validator.py` 精度 + `perf_compare.py` 性能 + `validate_acceptance_state.py` 三级完整性门）；本命令与 agent **不自行判 pass/fail**，只逐字引用确定性产物。链末端由统一工件边界分为正式 `acceptance.json`、非正式 `attempt_record.json` 或开发级 `dev_*`，三者不能互相顶替。
