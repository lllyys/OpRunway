...
**起因。** 三份真实社区任务书（roll、indexfilltensor、median）拿到手后，
生成侧一份都解析不了。`_taskdoc.py` 硬要 `§2.3` 的 `GetWorkspaceSize` 代码块与
`§2.4` 里同时含「参数名」「dtype类型」两列的表，而三份任务书都没有编号章节，
median 连参数表都没有——只有一句「支持 aclnnMedian、aclnnMedianDim 所有走入
aicore 的数据类型」。

这不是调参能修的，是解析层的架构假设错了：**社区任务书没有固定结构。**
...
下线的机制：`artifact-contracts.json` 骨架、`gate_lookup.py` 门禁清单、
`mark_step.py` 作战卡、`probe_progress.py` 进度反推、`seal_bundle.py` 封印、
`rewire_adapter.py` 受控改写、`_coverage_strategy.py` 分面拆分、
`align_signatures.py` 签名对齐、L0-L3 知识分层、行文散文门禁、双份共用件同步纪律。

这些都是为「零上下文 agent 不可信」设计的补偿机制，但它们消耗的上下文比防住的
返工更多，且全部建立在「任务书结构固定」这个已被证伪的前提上。

**新的分工。** agent 解析任务书，脚本校验结果。事实来源按三跳降级并逐项记来源：
任务书任意位置（`taskdoc`）→ 工程 `docs/aclnn*.md`（`opdoc`）→ 基线接口推断（`inferred`）。
任务书换个写法不影响脚本。

**底本。** 以 ATK submodule 自带的 `skill/atk-operator-onboarding`（402 行 workflow）
为骨干。它已经把「读文档 → yaml → 约束器 → 执行器 → atk case → atk task」写完了。
我们的真正增量只有四块：任务书 → 覆盖策略、母仓编译安装部署、golden 冻结与
accuracy_load 比对、精度性能裁决。

**一处与 ATK 自带 skill 相反的裁决。** `atk-quality-guard` 的 NEVER #4 要求
「从 C++ 源码提取 assert 写约束器」。那个 skill 服务已验收算子的质量加固，
目标是不产生假失败；本仓服务社区算子验收，目标是能发现它错。用被测算子自己的
断言生成用例，写错的断言永远测不出来。所以约束只从公开接口面取，
`op-engineering.md` 不采纳。

**真机结果（Atlas A3 / CANN 9.0.0-beta.1 / ATK 26.8.8）。**

| 算子 | 用例 | golden | 精度 | 性能形态 |
| --- | --- | --- | --- | --- |
| Roll | 180 | 180/180 | 180/180（100%） | `none` |
| IndexFillTensor | 200 | 200/200 | 见下 | `cross_dtype` |
| Median | 200 | 200/200 | 200/200（100%） | `builtin` |

IndexFillTensor 报出 47 条执行失败，隔离复验后只有 1 条是真的
（id=152，`int32 shape=[16,1,33] dim=1 index=[0]`，`ACL stream synchronize
failed, error code:507015`），其余 46 条是 aicore 异常后设备未恢复的连带失败。
这个发现促成了 `run_atk.py --mode isolate` 与 `verdict.py` 的连带失败识别——
不隔离就会把 1 条算子缺陷报成 47 条。

**开发过程中真机改掉的 skill 缺陷。**

| 缺陷 | 修法 |
| --- | --- |
| `aclnn_api_type` 写成 `pyaclnn` | 默认注册名是 `aclnn_function`（`case_config.py:91`） |
| YAML 的 `inputs` 把 `out` 也写了进去 | ATK 从 CPU 标杆返回值推输出，`inputs` 只列输入 |
| 冒烟子集叫 `smoke.json` 导致 golden 找不到 | golden 子目录名取自用例文件基名，改用 `smoke/cases.json` |
| `--op IndexFillTensor` 推出 `index_fill_tensor` 找不到目录 | 改按目录结构找，名字只用于消歧 |
| 母仓 `build/` 不清导致包里是上一个算子的符号 | 构建前默认清 `build/` 与 `build_out/` |
| 性能任务的 summary 表列名不同被读成 0 | 按 mode 分别解析两套列名 |
| 装出来的 opp 目录只读导致 rmtree 失败 | `_force_rmtree` 带 chmod 回调 |
| 取报告只取「最新」，任务挂到没产出报告时沿用上一轮的 | `_newest_report` 加时间下界，只收本轮产出的 |
| 两轮性能都写 `evidence/performance.log`，基线轮覆盖被测轮 | 日志名按 `--builtin-baseline` 分开 |
| 内置实现不支持新增 dtype，两轮无法在全 dtype 上比 | `sample_smoke.py` 加 `--dtypes` 过滤，报告写明覆盖范围 |

最后一条里的取报告 bug 值得单独说：它把 IndexFillTensor 的真实失败
（id=152，单独跑 5/5 必挂）判成了连带失败，报告写出「真实失败 0 条」。
一个只影响错误路径的取值 bug，直接把验收结论从「不通过」翻成「几乎全过」。
这类 bug 单元测试很难覆盖——它只在「任务挂到没写出报告」这个真实故障态下触发。

**另外两处只有真机能发现的问题。**

`case_config.id` 在 `after_case_config` 里恒为 0——id 是钩子跑完才赋的。
我照 ATK 自带 skill 的写法用 `case_config.id % N` 分批构造场景，结果全部用例
命中同一分支：Median 的 200 条**全部**变成退化维场景、dim 全取正轴。
条数、dtype 分布、门禁全绿，只有把秩分布和轴正负分布打出来才看得见。
所以 SKILL.md 现在明写「条数对不代表形态分布对」，并给了抽查命令。

冒烟门禁原本是「执行失败一条都不许有」。IndexFillTensor 冒烟 29/30 成功、
1 条失败，按原规则整个验收被拦死——但 29 条能跑说明部署没问题，那 1 条是
算子缺陷，正是要在 A4 测准的东西。改成失败率 > 20% 才拦：部署坏了会让绝大
多数用例跑不起来，低失败率天然不是部署问题。

**规则太严和太松一样有害。** 这两条都是把「本该报出来的缺陷」挡在了报告之外。
