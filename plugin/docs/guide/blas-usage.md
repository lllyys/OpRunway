...
- **判据三句话**：复测按 pass-once 裁——一个 case 在任一有效轮 PASS 即 PASS，永久有效；
  ...
  必须重跑 A5，否则不构成最终报告。
- **最小输入**：首轮工作目录 + 点名的 case；其余参数（run-id、轮号、默认卡、是否需要
...
- **用哪张卡**：默认沿用首轮那张物理卡，不用指定；首轮走 `auto` 时落回当时实际选中
  的那张。要换一张就在提示词里点名，例如把复测行写成
  `复测 case：TC_PF_012；用 6 号卡`。助手换卡前先跑机械预检确认这个工程能不能换：
  推不出该用哪个编译卡号时不给换，只用默认卡。预检只看这一件事，不查卡的忙闲。
  换卡的数值可比性只有一次单机单例证据：A3 机同一用例在两张卡上各采 3 次，中位数
  差 2.04%，低于当时定的 3% 比较线（**这条线是那次实验的判据，不是性能通过阈值**——
  性能仍按 `ratio ≥ 0.8` 裁）。换别的机型或别的用例时这个量级未经验证，
  跨卡数值出现意外差异先怀疑卡。
- **换卡要连着占位卡一起空出来**：换卡靠把目标卡映射到二进制编译时的那个卡位，
  映射串里排在目标卡前面的几张卡是占位用的，跑的时候用不到，但**必须空闲**。
  占位卡从 0 号起依次取、跳过目标卡，**不一定就是号码比目标卡小的那几张**——
  目标卡号小时会顺延用到更大的号。
  占位卡被别人占着时整轮跑失败，症状是逐例 `CRASH` 且没有 profiling 产物目录——
  实测里算子本身算对了（用例内部判定通过、精度误差为 0），失败卡在采集权限上，
  所以看到这个症状先查卡，别先怀疑算子。处置两条：改回同卡复测，或换一张目标卡，
  让它前面的占位卡全空。助手起跑前只查目标卡忙闲，占位卡查不了。
