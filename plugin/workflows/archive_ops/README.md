...
3. **symlink 可解析**：案例卡到 `specs/` 与 `new_example/` 的 symlink 必须能 `readlink` 解析（相对拓扑、不 sed 绝对路径）。
...
- [ ] 到 `specs/<op>.spec.json` 与 `new_example/oprunway_<op>_runner.cpp` 的 symlink 能 `readlink` 解析（相对路径、非绝对/私有）？
- [ ] 非「对应配错 / 空任务」作废案例？
