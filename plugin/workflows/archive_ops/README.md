...
3. **引用可解析**：`<op>.spec.json` 是**内联真实副本**（真样例已迁 `samples/specs/`、移出运行时路径，不再 symlink）；到 `new_example/oprunway_<op>_runner.cpp` 的 symlink 仍须能 `readlink` 解析（相对拓扑、不 sed 绝对路径）。
...
- [ ] `<op>.spec.json` 是内联真实副本（内容与 `samples/specs/<op>.spec.json` 一致、非 symlink）？到 `new_example/oprunway_<op>_runner.cpp` 的 symlink 能 `readlink` 解析（相对路径、非绝对/私有）？
- [ ] 非「对应配错 / 空任务」作废案例？
