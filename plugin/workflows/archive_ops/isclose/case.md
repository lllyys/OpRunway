...
| `evidence_path` | 历史台账已清理；本案例只作 archive，不能替代 current receipt/evidence |
...
- spec：[`isclose.spec.json`](./isclose.spec.json)（**内联真实副本**，源 `plugin/samples/specs/isclose.spec.json`；真样例已迁出运行时路径，不再 symlink）
