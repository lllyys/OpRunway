...
# 批 2：`load_golden` 的返回改具名元组——字段一多，位置解包（`a, b, c, d = load_golden(op)`）就成了
# 可读性灾难，且再加字段时**旧调用点会静默错位**而不是报错。具名元组既保位置兼容、又能按名取。
Golden = collections.namedtuple("Golden", "fn source provenance out_shape contract")


def load_golden(op):
    """按算子名从用户侧加载 golden——`<ops_root>/<op>/golden.py`，返回
`Golden(fn, source, provenance, out_shape, contract)` —— **具名元组**，位置解包仍兼容。
