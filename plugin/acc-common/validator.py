...
def _verdict(op, vm, spec_standard, problems, per, gaps=None, scaled=None, golden_tiers=None,
    ...
    return {"op": op, "verify_mode": vm, "standard": spec_standard,
            ...
            "overall": {"verdict": overall, "uncertain": unc_ids, "risk": risks,
                        ...
                        "golden_judged_from": golden_judged_from,
...
def validate(spec, caseset, evidence):
    ...
    if not isinstance(cases, list) or not cases or not isinstance(ev_list, list):
        # 结构性致命 → 直接出 fail verdict（不崩）。golden_judged_from：spec 有 golden 键但用例都没了 →
        # 这条 golden 链没被判过（codex Low #9），标 not_judged 而非谎称 legacy/spec。
        return _verdict(op, vm, None, problems, [],
                        golden_judged_from=("not_judged" if "golden" in spec else "caseset_self_declared"))
    ...
    # 批 4（硬约束 #5）：收**原始** per-case golden_tier（不去重、保留 None/非dict）——
    # 去重与「缺失怎么处置」都交给 _reconcile_golden。spec.golden 在场时「某 case 没带 golden_tier」
    # 是删改信号、必须 fail-closed（codex + 红队独立逮到同一 Critical：空集合让门整个不触发 → 静默 pass），
    # **不能像 legacy 那样静默跳过**。
    _raw_tiers = [((c.get("expected") or {}).get("golden_tier") if isinstance(c, dict) else None)
                  for c in cases]
...
def _norm_sha(v):
    """SHA 规范化，与 `precision_policy.verify_authorization` 的 `strip().lower()` 对齐——
    否则 caseset 发射一个带大写/外围空格的**合法** SHA，会与 spec 规范化后的 sha 对账假 fail。"""
    return v.strip().lower() if isinstance(v, str) else v


def _validate_spec_golden_anchor(sg):
    """校 spec.golden 结构（判据锚），返回 `(anchor|None, problems)`。

    spec.golden 是**判据权威**——它畸形 / 缺 sha 时绝不能 fail-open 去信 caseset，必须当场 fail-closed。
    返回 `anchor=None` 即「锚不可信」，调用方据此合成 blocked（codex 审 High #3/#4/#5：畸形 spec.golden
    被误当 legacy 或 truthy 非 dict 触发 AttributeError 逃出 validate）。全程 `isinstance` 守护、不抛。"""
    if not isinstance(sg, dict):
        return None, [f"spec.golden 须为对象（判据锚），得 {type(sg).__name__}——fail-closed，不降级信 caseset"]
    problems = []
    src, mk = sg.get("source"), sg.get("method_kind")
    auth = sg.get("authorization")
    kind = auth.get("kind") if isinstance(auth, dict) else None
    if not isinstance(auth, dict):
        problems.append(f"spec.golden.authorization 须为对象，得 {type(auth).__name__}")
    if src not in precision_policy.GOLDEN_SOURCE_KIND:
        problems.append(f"spec.golden.source={src!r} 不在受控词表")
    if mk not in precision_policy.GOLDEN_METHOD_KIND:
        problems.append(f"spec.golden.method_kind={mk!r} 不在受控词表")
    if kind not in precision_policy.AUTHORIZATION_KIND:
        problems.append(f"spec.golden.authorization.kind={kind!r} 不在受控词表")
    snap = sg.get("taskdoc_snapshot")
    sha = None
    if kind in ("oracle_method", "formula"):
        # 声称任务书授权 → 必须有合法 64-hex 快照指纹，否则「授权」无从对账（红队 High：缺 sha → None==None 假通过）
        if not isinstance(snap, dict):
            problems.append(f"spec.golden 声称任务书授权({kind}) 但缺 taskdoc_snapshot 对象")
        else:
            sha = _norm_sha(snap.get("sha256"))
            if not (isinstance(sha, str) and _SHA256_RE.match(sha)):
                problems.append(f"spec.golden.taskdoc_snapshot.sha256 须 64 位小写十六进制，得 {snap.get('sha256')!r}")
    elif isinstance(snap, dict):
        sha = _norm_sha(snap.get("sha256"))
    anchor = {"source": src, "method_kind": mk, "authorization_kind": kind, "snapshot_sha": sha}
    return (anchor if not problems else None), problems


def _blocked_tier(reason, note):
    return {"tier": 4, "requires_human_review": True, "blocked_reason": reason,
            "judged_from": "spec", "note": note}


def _reconcile_golden(spec_has_golden_key, spec_golden, raw_tiers):
    """判据锚拉回 spec（硬约束 #5）。返回 `(effective_tiers, problems, judged_from)`。

    ⚠ **第一版被 codex + 红队各自独立打穿了同一个 Critical**（spec.golden 在场时删掉 caseset 的
    golden_tier → 收集空集合 → 门整个不触发 → 静默 pass，还谎标 judged_from="spec"）。重写后的铁律：
      **spec.golden 是判据权威；caseset 的 golden_tier 只用于「逐项核对 + 供 authorization_verified」，
      从不用来决定 blocked 与否。** 任何删改 / 畸形 / 不一致一律 fail-closed 成 blocked，绝不静默放行。

    三条路径：
      A. **spec 无 golden 键**（真正的 legacy）→ caseset 自声明原样判（同批 4 前），judged_from=caseset_self_declared。
      B. **spec 有 golden 键但畸形**（非 dict / 词表错 / oracle 缺合法 sha）→ **fail-closed 成 blocked**，不降级信 caseset。
      C. **spec.golden 合法** → 从它派生一次权威档：
         - 每条 case 必须带 dict golden_tier（缺 / 非 dict = caseset 被删改 → problem）；
         - 逐 case 对账 anchor 四字段（含规范化 sha）一致；`authorization_verified` 严格布尔且**全体一致**；
         - authoritative_av = 无 problem 且 av 唯一时那个值，否则 False（fail-closed）；
         - 用 spec 锚 + authoritative_av 重新 `derive_golden_tier`；有任何 problem 而派生却非 blocked → 强制 blocked。

    ⚠ 残余边界不变（documented）：`authorization_verified` 仍取自 caseset（纯函数复现不了读快照核引文），
       对账 sha 把它钉到 spec；收窄到「真快照在场 + sha 对 + 引文不逐字」那一窄缝。见 test_residual_boundary_documented。"""
    declared = [t for t in raw_tiers if isinstance(t, dict)]

    if not spec_has_golden_key:                       # 路径 A：真正的 legacy
        seen, eff = set(), []
        for t in declared:
            try:                                      # 展示用去重；字段可能不可哈希（攻击者塞 list）→ repr 兜底、别让 validate 崩
                k = (t.get("tier"), t.get("blocked_reason"), bool(t.get("requires_human_review")))
                hash(k)
            except TypeError:
                k = repr(sorted((str(kk), str(vv)) for kk, vv in t.items()))
            if k in seen:
                continue
            seen.add(k); eff.append(t)
        return eff, [], "caseset_self_declared"

    anchor, sg_problems = _validate_spec_golden_anchor(spec_golden)
    if anchor is None:                                # 路径 B：spec.golden 畸形 → fail-closed，不信 caseset
        return [_blocked_tier("needs_user", "spec.golden 结构非法")], sg_problems, "spec"

    # 路径 C：spec.golden 合法 → spec 权威，caseset 只核对
    contract = {"source": anchor["source"], "method_kind": anchor["method_kind"],
                "authorization": {"kind": anchor["authorization_kind"]},
                "taskdoc_snapshot": ({"sha256": anchor["snapshot_sha"]} if anchor["snapshot_sha"] else None)}
    problems = []
    if len(declared) != len(raw_tiers):               # ⭐ Critical 堵点：spec 说有 golden 判据，caseset 却缺/畸形
        problems.append(
            f"spec.golden 在场，但 {len(raw_tiers) - len(declared)}/{len(raw_tiers)} 条 case 缺 golden_tier "
            f"或非对象——caseset 被删改（判据锚说本算子有 golden 判据），fail-closed")
    avs = set()
    for t in declared:
        for name in ("source", "method_kind", "authorization_kind", "snapshot_sha"):
            tv = _norm_sha(t.get(name)) if name == "snapshot_sha" else t.get(name)
            if tv != anchor[name]:
                problems.append(
                    f"golden 判据锚不符（硬约束 #5）：spec.golden.{name}={anchor[name]!r} ≠ "
                    f"caseset 声明 {name}={t.get(name)!r}——改 caseset 想绕过 golden 门会撞这道对账")
        av = t.get("authorization_verified")
        if av is not True and av is not False:        # 严格布尔（同 derive_golden_tier 的安全边界）
            problems.append(f"caseset 的 authorization_verified 非布尔（{type(av).__name__}）——按未核实处理")
            av = False
        avs.add(av)
    if len(avs) > 1:                                  # 同一 golden 的核验结果只能有一个真相
        problems.append(f"caseset 内 authorization_verified 不一致 {sorted(avs)}——fail-closed")

    authoritative_av = avs.pop() if (len(avs) == 1 and not problems) else False
    re_tier, re_needs, re_blocked = precision_policy.derive_golden_tier(contract, authoritative_av)
    if problems and not re_blocked:                   # 有篡改/删改信号但派生没 blocked → 强制 blocked（fail-closed）
        re_tier, re_needs, re_blocked = 4, True, "unverifiable_authorization"
    eff = {**(declared[0] if declared else {}), "tier": re_tier, "requires_human_review": re_needs,
           "blocked_reason": re_blocked, "judged_from": "spec"}
    return [eff], problems, "spec"
