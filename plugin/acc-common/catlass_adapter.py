...
def _metric(verify_mode, out, golden):
    """采集误差值（不判 pass/fail）。matmul 走 numerical → max_rel_err。"""
...
def build_matmul_caseset(spec, work_dir):
    ...
    threshold = spec.get("precision", {}).get("threshold")
...
def parse_results(ctx, outs, perfs, defect_cases=None):
    """⑥ parse_results —— 组装 evidence（精度 metric + perf）。schema 对齐 repo_adapter run_mock/new_example。"""
    wd = ctx["work_dir"]
    ev = []
    for c in ctx["cases"]:
        cid = c["id"]
