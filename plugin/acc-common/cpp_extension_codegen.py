...
DISPATCH_STANDARD_STATUS = "generated_standard_two_stage_status"
    ...
    for i, v in enumerate(variants):
        ...
        for combo_index, combo in enumerate(scalar_combinations):
            ...
            dispatch = (
                ...
                else DISPATCH_STANDARD_STATUS if stage2_form == STAGE2_STANDARD
...
def _render_generated_two_stage_body(
    ...
    OpRunwayWriteCallStatus(workspace_status, workspace_size, executor == nullptr, false, 0);
    if (workspace_status != 0 || executor == nullptr) {{
        ReleaseConvertTypes(converted_params);
        UnInitExecCommonCtx();
        TORCH_CHECK(false, "{symbol}GetWorkspaceSize failed, ret=", workspace_status,
                    ", executor_null=", executor == nullptr);
    }}
...
{exec_block}
    ...
    auto acl_call = [exec_params, converted_params, workspace_tensor, op_api_func,
                     acl_stream, workspace_status, workspace_size]() -> int {{
        (void)workspace_tensor;  // 捕获只为续命：buffer 必须活过下面这次执行。
        InitExecSubTheadCtx(acl_stream);
        auto api_ret = call(op_api_func, exec_params);
        OpRunwayWriteCallStatus(workspace_status, workspace_size, false, true, api_ret);
        ReleaseConvertTypes(converted_params);
        UnInitExecCommonCtx();
        return api_ret;
    }};
    ...
    for v in variants:
        ...
        if v["stage2_form"] == STAGE2_STANDARD:
            body = _render_generated_two_stage_body(
                ...
                standard_abi=True, explicit_conversion=explicit_conversion)
...
namespace {{
void OpRunwayWriteCallStatus(int64_t stage1_ret, uint64_t workspace_size,
                             bool executor_null, bool stage2_called,
                             int64_t stage2_ret) {{
    const char *path = std::getenv("OPRUNWAY_CPP_EXTENSION_CALL_STATUS");
    if (path == nullptr || path[0] == '\\0') {{
        return;
    }}
    std::ofstream out(path, std::ios::out | std::ios::trunc);
    TORCH_CHECK(out.good(), "cannot write OpRunway call status");
    out << "{{\\\"schema\\\":\\\"oprunway.cpp_extension_call_status\\\","
        << "\\\"schema_version\\\":1,\\\"stage1_ret\\\":" << stage1_ret
        << ",\\\"workspace_size\\\":" << workspace_size
        << ",\\\"executor_null\\\":" << (executor_null ? "true" : "false")
        << ",\\\"stage2_called\\\":" << (stage2_called ? "true" : "false")
        << ",\\\"stage2_ret\\\":";
    if (stage2_called) out << stage2_ret; else out << "null";
    out << "}}" << std::endl;
    TORCH_CHECK(out.good(), "cannot flush OpRunway call status");
}}
}}  // namespace
