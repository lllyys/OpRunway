...
def _canonical_state(overall, ps):
    """人读 overall → 机读 canonical 状态。门因不可比/挂起而 FAILED 时据 perf status 细化，
...
def _exit_code(overall):
    """退出码枚举（与 T5 同一套；修 startswith('PASS') 潜伏 bug——PASSED_WITH_RISK 曾被误判 exit0）：
      0 = 干净 PASS / PASS(无性能要求)；2 = PASSED_WITH_RISK（requires_human_cp，CI 挂起转人工）；
      1 = 其余（FAIL 精度 / 性能未达 / BLOCKED_* / NEEDS_REVIEW）。
    ⚠ 合并：这是 T5 也会加的同款枚举；若 T5 已加，用 T5 的即可（语义等价）。"""
...
def run(spec_path, mode="mock", out_dir="reports/_run", defect=None, perf_slow=None, gpu_baseline=None):
    ...
    state = _canonical_state(overall, ps)   # 机读 canonical 状态（人读串仍 overall）
    exit_code = _exit_code(overall)         # 退出码枚举：0 干净 / 2 PASSED_WITH_RISK / 1 其余
    print(f"[总体] 精度={prec} · 性能达标 {ps.get('达标')}/{ps.get('perf_cases')}({ps.get('status')}) "
          f"· 门={'PASSED' if gate_passed else 'FAILED'} → {overall}"
          + ("（requires_human_cp）" if requires_human_cp else ""))
