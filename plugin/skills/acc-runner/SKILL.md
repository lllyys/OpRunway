---
name: acc-runner
description: OpRunway 验收的代码产出 skill，两件事：(a) gen_golden——据任务书为任意算子产 <ops_root>/<op>/golden.py（真值口径走两档链、PR/仓内参考实现禁作 golden 源、后端生成期定死、check_golden.py 自检），纯 CPU、不需 NPU、不受 runner scope gate 约束；(b) gen_runner/verify_runner——为 ops-<族> 仓、aclnn 两段式接口、opp 安装型的算子（含非 experimental 子树，引擎目录/后缀已生成化），据 spec + PR 事实（算子自带 example + op_def）生成一个「锚定算子实测路径」的 per-op NPU runner（oprunway_<op.lower()>_runner.cpp），并按 runner 自检证据满足/不满足 纪律（未满足则停在 CP-C、不上真机）后才交真机跑测。OpRunway 验收 ③：spec 已就绪、要在真 NPU 上跑一个此类算子的正确性/性能时用。catlass（换构建体系）/非 aclnn 接口/双实现 当前不支持（需先扩 adapter，见 dev-doc/oprunway-batch6b-design.md）。⚠ gen_runner/verify_runner 服务的 runner_form=cpp 自 2026-08-06 已停止准入、无真机入口，机制仅作历史保留；正式验收统一走缺省的 cpp_extension（AGENTS.md §4）；gen_golden 不受此限。
---
...
缺 runner 直接 **fail-closed** 报错，真机 `new_example` 模式（= `spec.runner_form` **显式**为 `cpp` 时**派生**出的那个 mode）`runner_source` 恒 `user`、非 user 一律 `BLOCKED`。

⚠ **先分清「能跑」与「能出裁决」**：`spec.runner_form` 受控词表为 `{cpp, aclnn_py, cpp_extension}`、**缺省 = `cpp_extension`**，而**验收裁决当前只出自 `cpp_extension`**（`run_workflow._ACCEPTANCE_RUNNER_FORMS = frozenset({"cpp_extension"})`，入口门 `_resolve_mode` + 出口门 `_assert_acceptance_form_allowed` 两道；理由见仓根 `AGENTS.md` §4）。`cpp` / `aclnn_py` 已于 2026-08-06 **停止准入**：派生表里没有它们的条目、逃生阀 `--allow-experimental-form` 已删，显式 `--mode new_example` / `aclnn_py` 同样被拒——**连跑都跑不起来**。所以本页 `gen_runner` / `verify_runner` 那套**仅作历史保留**（解释旧产物怎么来的）；要正式验收须走 `cpp_extension`。**别问用户走哪条 form**：form 由 spec 派生，验收统一按 `cpp_extension`。⚠ 历史产物保持原裁决与历史效力，停止准入 = 不支持新建。

<!-- oprunway:retired-begin -->
⛔ 历史留档 · 不得 dispatch · 不要照做 —— **`spec.runner_form == "aclnn_py"` 不适用上面这条规则**（同为已停止准入的形态，本段只解释历史产物）：该形态**没有 per-op runner 源**（op 工程即 DUT，跑的是完全 op-中立的通用 ctypes runner），`find_runner()`/`runner_source` 那套无从谈起；当年它派生出的是 `--mode aclnn_py`，那一行已从派生表删除、跑不起来。当年其信任门由 `acceptance-workflow` CP-C 的「`aclnn_py` harness 真机自检」接住——**「无源可自检」≠「免验证」**，这条纪律今天由 `cpp_extension` 的 vendor 构建收据继承。
