---
name: acc-runner
description: OpRunway 验收的代码产出 skill，两件事：(a) gen_golden——据任务书为任意算子产 <ops_root>/<op>/golden.py（真值口径走两档链、PR/仓内参考实现禁作 golden 源、后端生成期定死、check_golden.py 自检），纯 CPU、不需 NPU、不受 runner scope gate 约束；(b) gen_runner/verify_runner——为 ops-<族> 仓、aclnn 两段式接口、opp 安装型的算子（含非 experimental 子树，引擎目录/后缀已生成化），据 spec + PR 事实（算子自带 example + op_def）生成一个「锚定算子实测路径」的 per-op NPU runner（oprunway_<op.lower()>_runner.cpp），并按 runner 自检证据满足/不满足 纪律（未满足则停在 CP-C、不上真机）后才交真机跑测。OpRunway 验收 ③：spec 已就绪、要在真 NPU 上跑一个此类算子的正确性/性能时用。catlass（换构建体系）/非 aclnn 接口/双实现 当前不支持（需先扩 adapter，见 dev-doc/oprunway-batch6b-design.md）。⚠ gen_runner/verify_runner 服务的 runner_form=cpp 当前是非准入形态——机制仍有效但产不出验收裁决（要跑须加 --allow-experimental-form），正式验收统一走缺省的 cpp_extension（AGENTS.md §4）；gen_golden 不受此限。
---
...
⚠ **先分清「能跑」与「能出裁决」**：`spec.runner_form` 受控词表为 `{cpp, aclnn_py, cpp_extension}`、**缺省 = `cpp_extension`**，而**验收裁决当前只出自 `cpp_extension`**（`run_workflow._ACCEPTANCE_RUNNER_FORMS = frozenset({"cpp_extension"})`，入口门 `_resolve_mode` + 出口门 `_assert_acceptance_form_allowed` 两道；理由见仓根 `AGENTS.md` §4）。`cpp` / `aclnn_py` 仍跑得起来（须 `--allow-experimental-form`），但物理上只产 `dev_run_summary.json` / `dev_precision_check.json`（`evidence_grade="development"`），**不写** `acceptance.json` / `verdict.json`。所以本页 `gen_runner` / `verify_runner` 那套**只服务开发级路径**（修通路、复现问题、局部开发验证）——机制照旧有效，但产物进不了验收裁决；要正式验收须走 `cpp_extension`。**别问用户走哪条 form**：form 由 spec 派生，验收统一按 `cpp_extension`。

⚠ **`spec.runner_form == "aclnn_py"` 不适用上面这条规则**（⛔ 同为非准入形态，以下机制只在开发级路径下有效）：该形态**没有 per-op runner 源**（op 工程即 DUT，跑的是完全 op-中立的通用 ctypes runner），派生出的是 `--mode aclnn_py`，`find_runner()`/`runner_source` 那套无从谈起。其信任门改由 `acceptance-workflow` CP-C 的「`aclnn_py` harness 真机自检」接住——**「无源可自检」≠「免验证」**。
