...
- **注册 ≠ 接线（P2 诚实边界）**：P2 把本 skill 列进 `plugin/AGENTS.md` 的 `skills:` **注册清单**（供跨 CLI 分发 / 发现），**但这只是「plugin 对外暴露该能力 skill 供人或 subagent 组合」，并不代表已接进 `op-acceptance` agent 的 runtime 调用链**——live 用例生成仍走 `gen_cases.py`，本 skill 勿被自动触发。真正接进运行路径属 P1 / 后续，非 P2。
- **通用 generator 是路线图、非当下能力**：`gen_cases.py` 现为**硬注册** `GOLDEN={IsClose, Sign, Equal, Neg}`（只对已注册算子确定性产 caseset），**不是** rule-catalog 驱动的通用 generator。「rule-catalog → 通用 generator」是 canon 目标态 / 独立后续 todo，本 skill **不声称当下可对任意算子自动落 caseset**。
