...
**定位（诚实）**：本 skill 是落地设计 **P2 规划的原子 skill**，承载 canon `Primitive-to-case rule library`（canonical）里「展开逻辑 = SKILL.md、目录 = `references/rule-catalog.md`」划分中的**展开逻辑**部分。**当前尚未接入 op-acceptance live 编排流**：真实运行时的用例生成走确定性脚本 `plugin/acc-common/gen_cases.py`——**本 skill 不落盘 `caseset.json`、不调用也不替代 `gen_cases.py`**，只描述「一个算子该测哪些用例、为什么」的推理规则。故本 skill **未列入 `plugin/AGENTS.md` 的 skills 已接入清单**（P2 拆原子 skill 时再正式接入）。
