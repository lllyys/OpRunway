...
1. **取材**（确定性活，下放给脚本）：
   ```
   python3 ${OPRUNWAY_PLUGIN_ROOT:-$CLAUDE_PLUGIN_ROOT}/acc-common/fetch_source.py --taskdoc <路径|链接> --pr <PR链接> --out <workdir>
   ```
   > 插件根用**中立主变量 `${OPRUNWAY_PLUGIN_ROOT}`**；`:-$CLAUDE_PLUGIN_ROOT` 是自兜底——Claude 下 harness 只设 `CLAUDE_PLUGIN_ROOT`，回落即可跑；**Codex 等非 Claude 运行时须先 `export OPRUNWAY_PLUGIN_ROOT=<插件根绝对路径>`**。
...
2. **抽 spec**（本 skill 的 NL 判断核心）：读 `task_doc.md` + `pr_facts.json`，按 `references/taskdoc-to-spec.md` 的**字段映射表**逐字段抽。
   ...
   四个要点（都在 ref 里，最易错）：
   - **公开身份与内部执行身份分开**：`spec.op` 只写任务书/API 的公开算子身份；
     `spec.execution` 必须逐字复制本轮 `source_facts.payload.derived.kernel_identity` 的唯一
     `OP_ADD(<Identifier>)` 候选及 `source_binding`。不得从公开名、目录或 API 名猜内部类型；
     候选为 0/多项时停在 CP-A，不能产 current spec。确定性读侧仅为既有同名/current spec
     保留“字段缺席且 source fact 恰为单候选”的派生兼容；这不授权 extractor 省略新字段。
   - **🔴 `runner_form` 正式验收一律抽 `cpp_extension`**（ref §1.3.1）：`cpp` / `aclnn_py` 自 2026-08-06 **停止准入**——`run_workflow._RUNNER_FORM_TO_MODE` 里没有它们的条目，抽成那两个值等于交出一份**开跑即被拒**的 spec，后面的 golden、dry-run、CP-C 准备全白做。⚠ **别拿「被测物是 aclnn 两段式」反推 `aclnn_py`**（那判的是域内/域外，不是执行形态，见下面 §1.3 那条红线）；⚠ 更别因为「这条通路跑不通」回头改 `runner_form` —— 改 spec 缩范围是本仓明令禁止的动作，正确处置是补 torch.ops 调用桥 + vendor ELF 构建收据把它迁到 `cpp_extension`（成本更高，这是已知账单）。
   - **dtype 全集按任务书语义选择本轮事实源**：任务书显式枚举 dtype 时逐字采用；任务书若把范围定义为“本算子所有进入 AICore 的数据类型”等实现域集合，则本轮从同一 PR head 的 `op_def` 与目标硬件 `AddConfig` 枚举集合成员——任务书负责定义集合，op_def 负责给出成员。不得因任务书没有重复抄枚举而询问用户或阻塞，也不得复用旧报告中的 dtype。只有任务书既未枚举、也未给出能绑定到 op_def/独立 built-in 能力源的集合定义时才问用户。PR op_def 不能扩张任务书明确写窄的范围；两者冲突时仍以任务书为准并记录 gap。`params.dtype` 只填当前 pipeline 与本轮 runner form 支持的真机可测子集，其余逐项挂账。
   - **dtype 覆盖门字段**（Q7，ref §1 dtype_required/dtype_tested 行）：`dtype_required`=任务书**权威全集**（来源同上：任务书表 > 信息库 > 问用户；全集未知/信息库未接通→字符串 `"needs_user"`；legacy 未迁→省略）；`dtype_tested`=实测子集（gen_cases 据**真实生成的 cases** 归并；门用真实 cases 对账，自报不符即 BLOCKED）。**required 有、tested 无的 dtype 必须逐个挂账**，两类二选一（ref §1.2 有对照表）：**我们测不了** → `{kind:"dtype_deferred",dtypes:[…],capability_source:"generation|runner|compute",runner_form:…(仅 runner 来源),reason:…}`；**算子 op_def 压根不支持**（C4）→ `{kind:"dtype_unsupported_by_op_def",dtypes:[…],task_doc_ref:…,op_def_ref:…,op_def_dtypes:[…]}`。挂账合规 → 门不判「静默收窄」，无记录则 BLOCKED。⚠ **「挂账」≠「免检」**：`dtype_deferred` 须声明**是哪张能力表**不支持并接受与活表的交叉核验（自报不支持、表里其实支持 → 拒该 gap → 仍 BLOCKED），且其终态**不会**是干净 `pass`（至少 needs_review）——四道硬校与终态映射见 ref §1.2a。IsClose 已核全集={float32,float16,bfloat16,int32}；runner/codec 已接入 bf16（**仅 mock 通路跑过——mock 是非验收证据，不构成真机验证**），但 **bf16 真机验收阻塞在 op-build 环境**，故 IsClose 现 **tested={fp32,fp16}、bf16+int32 均 deferred**（bf16 待真机 op-build 恢复+kernel 验、int32 待 runner int 分支）。
   - **dtype 冲突以任务书为准**（C4，用户 2026-07-22 拍板，详规 ref §1.2）：任务书声明的 dtype 全集当需求（进 `dtype_required`），**算子 `op_def` 支持不了的差额入 `task_pr_gaps`**、裁决落 `passed_with_gaps`。「没实现」是**发现**、不是借口（见仓根 `AGENTS.md` §1、§5.1）。⚠ 这是「**绝不信 PR**、dtype 全集只来自任务书」这条红线的**延伸、不是例外**。⚠ 更不是「宣称有 gap 就免检」的后门——`validator` 四道硬校：**有据**（`task_doc_ref` 指任务书原文 + `op_def_ref` 指 op_def 出处 + `op_def_dtypes` 自报支持集，缺一即 `overall=fail`）/ **自洽** / **不得覆盖真失败**（该 dtype 有真实用例在跑 = 「实现了但跑挂了」，必须走精度裁决）/ **在 `dtype_required` 内**。
   ...
   - **precision.standard + tolerance_policy_id**（T5，待散文门）：`oracle` 只回答“谁产真值”，`standard` 独立回答“按哪套验收尺裁”。**任务书显式精度条款优先**：点名 AscendOpTest 默认阈值就取 `ascendoptest_default`，即使功能真值是 torch；只有任务书未指定精度工具/标准时，才据 oracle+verify_mode 走 ref §1.1 兜底映射。⚠ 两层 id 别混：**spec 级** `tolerance_policy_id` 是摘要/向后兼容（无 dtype 后缀），**caseset 级** `expected.tolerance_policy_id` 才是门控口径（格式 `standard:dtype`，由 `gen_cases` 派生）。`threshold` 现仅是向后兼容 digest（真门控走结构化 policy，见 ref §3）；per-case 结构化 policy 由 `gen_cases` 按 golden dtype 自动派生，不用手写。`ecosystem_mere_mare` 为 proposed/NOT_SETTLED——单标杆不过记 needs_review、不自动 fail。任务书验收目标明确宽于平台底线时才加可选 `acceptance_policy`。
   - **precision.case_target**（精度用例数）：**必填、无缺省**——不写这个键，`gen_cases` 真跑与 `--dry-run` 都当场 fail-fast（**须 ≥1**，0/负/非整同样 fail-fast）。**按覆盖矩阵算，不是拍一个数**：`case_profile=="torch_parity"` 时必须**精确等于** `dtype × rank × shape_profile × attribute_profile`；其它档按该算子覆盖轴推算，并把算法或沿用来源写进 `precision.case_target_source`；给不出依据就**停下问用户，别随手填一个数**。⚠ 原「缺省 50 / `AskUserQuestion` 问用户（建议 50）」已于 2026-08-06 删除——那个默认值让「用例数没人定过」静默通过（实测：extractor 自己填 50、0 次被审视，792 个候选组合留 50），且它要求的 `AskUserQuestion` 根本不在 `acc-spec-extractor` 的 `tools:` 里、物理上执行不了。详见 ref『case_target 怎么定』。
   - **附带自测的权威角色**（N5）：当用户/当前计划显式裁定「正式 caseset 与 golden 由 OpRunway 生成，任务书附件和源仓 selftest 只作 reference」时，必须同时写 `precision.case_source="generated"` + `precision.reference_case_material_role="reference_only"`。这是对「看到 self_test 链接就强制 taskdoc 档」旧启发式的**显式权威覆盖**：附带材料可取材留档/提取有引用的 requirement，但不得进 planner、不得决定 `case_target` / case 身份 / golden。省略显式 generated、绑 `precision.taskdoc_caseset`、或与 `taskdoc` 档组合都由 `gen_cases` 当场 fail-closed。无这项显式裁定时才沿用 ref §1.6 的历史两档规则。
   - **轴集契约**（ref §1.3「轴集契约」；数字源自设计记录 §12.2/§12.6/§12.8）：只有两条轴**共同决定实现分支或切分决策**时才必须交叉；只改数值量级、不改控制流则边际覆盖。实测正例是 `dtype` 类 × 归约长度共同圈定 90 个 cell，58 条 fail 全在区内、区外 0，合法 1-wise 集可 **100% 漏报**；反例是 `keepDim` 在 fail 区内 72.7% vs 81.8%、无可测效应。规则只据 spec 已有的接口能力 / `operator_class` 字段，**绝不按算子名**。三条边界同时守住：① 特殊场景不进笛卡尔、只独立叠加（避免组合爆炸，代价是「空 Tensor × 按维归约」永远测不到；`torch_parity` 是否应从当前零特殊场景改为独立保留 legacy 边界，**待用户拍板**）；② 多输出是 attr → `call_variants.active_outputs` 的函数，不是自由轴；③ 🔴 值域 regime 暂不引入——当前只有 `uniform`，结构上测不了交互、**零实测支撑**，引入前先有可测的多档矩阵，否则只是用例数翻倍而收益未知。
   - **runner 锚定线索**：先读 `source_facts.payload.derived.api_mapping`，其 `status` 必须是 `exact`；按任务书出现顺序消费完整 `aclnn_entries` / `targets`，逐 API 只认唯一 public header + 唯一 example 的精确交叉。**不得再从 `key_files` 排序后挑第一个 `test_aclnn_*.cpp`**：同目录的 TensorTensor / TensorScalar / inplace 会把 runner 锚到别的 overload。任务书点名多个 API 时全部保留，不得只取首个；0 命中或多解均停在 CP-A，不靠算子名/路径猜。输入 dtype 再从这些**已定身份**的 example 与 op_def 交叉。
   - **torch 对标 / 多输出 / aclnn_py 形态**（ref **§1.3**，只有这条通路才用得到的一组字段；不属该场景就**一个都别写**，全部可省略=现行 cpp 通路、行为零变更）：
     四件事**各自独立触发、不打包**（§1.3.0）——① 任务书把 **torch 立为真值口径** → `precision.oracle=torch`，并写 `runner_form="cpp_extension"`；`precision.standard` 仍按任务书显式精度条款独立抽，只有条款缺失才兜底为 `torch_allclose`。DUT 走官方 `NpuExtension + EXEC_NPU_CMD_EXT` 注册的独立 `torch.ops` namespace，stock `torch.*` 只作 golden/性能 baseline；除非任务书明确要求替换原生 dispatch，禁止改走 op-plugin 重编。② 被测来源是**标准 aclnn 两段式工程**（仓根 `build.sh` + 算子目录有 `op_host/` + **在 `<op_subdir>` 下有界递归**能找到 `aclnn_*.h`，无 opaque descriptor）**只判域内/域外，不改 `runner_form`**——🔴 **「被测物是 aclnn 两段式」不蕴含 `runner_form="aclnn_py"`**（旧文写反过，是 Roll 被判成 `aclnn_py`、卡在准入门前的直接原因，ref §1.3.1）；正式验收一律 `cpp_extension`，`call_variants` 照样必填，dtype 白名单据 form 取。①②同时成立写 `scenario="torch_ref_aclnn"`；③ 多输出 → 逐 out 写 `out_role`（受控词表 `value|index`），**每个 `index` 输出必填 `index_of` + `gather_from`**，下标不逐位比，判据是 `index_value_consistency`；④ `call_variants`——`runner_form ∈ {"aclnn_py","cpp_extension"}` 一律必填，`when`/`symbol`/`active_attrs`/`active_outputs` 明示，按 attr 分派、绝不按算子名。
   - torch 对标用例同时写 `precision.case_profile="torch_parity"` 与 `precision.torch_parity_matrix`。矩阵来源优先复用本轮取材可验证的 coverage 轴并记录 SHA-256；必须完整覆盖 dtype×rank×shape_profile×attribute_profile。逐个枚举任务书要求的 Torch overload，并与本轮 PR-head ABI 核对：可执行 overload 落 profile + `call_variants`；任务书要求、但 PR 没有可执行 ABI 的 overload，写结构化 `task_pr_gaps`，`kind="api_surface_unsupported_by_pr"`，附 taskdoc/header 引用和缺失调用形态。**不得伪造 call variant，也不得把“被测 PR 没实现”误报成“任务书事实不足”而阻塞 CP-B。**后续报告必须把该功能缺口纳入非 PASS 结论，已执行子集不得宣称完整 Torch 对标。
     配套三项：`precision.tolerance_source`（`dtype_table`(缺省)/`taskdoc`(须带 `taskdoc_tol:[rtol,atol]`)/`torch_default`，**只有整字段省略才落缺省**，§1.3.4）、`precision.value_profiles`（`["nan","tie"]` 子集，压 NaN 传播与并列下标；**声明了产不出即 fail-closed**；⚠ **`nan` 只配 `operator_class=="floating_compute"`**，其它类别声明它即 fail-closed，见下条）、性能 baseline 按**任务书事实 + 已记录用户确认**取：框架级 Torch 或已确认“小算子拼接等价于 Torch 接口”写 `perf.baseline="torch_npu"` + `perf.torch_baseline`；实际要求直接 ACLNN 才写 `perf.baseline="aclnn_builtin"` + `perf.aclnn_baseline`（§1.3.5）。
...
4. **自检**：按 ref §7 校验（verify_mode 合法、numerical 有 threshold、params 有 out、exact⇒threshold=0、**`precision.case_target` 显式在且 ≥1 且给得出依据**、**轴集只按接口能力判交叉，特殊场景/多输出/值域 regime 没被误加成自由轴**、add_dtype⇒dtypes_added 非空且其中 pipeline 支持项已并入 params.dtype、不支持项只记 gap、**C2 attr 值类型合法（含 `list[int]` 限制）**、**C3 `rank` 合法且确有依据**、**C4 dtype 冲突 gap 四字段齐全且其 dtype 不在 `params.dtype`**、**C1 spec 里没有自造的输出形状字段**、**§1.3.7 torch 对标/多输出/aclnn_py 形态自检（写了那组字段才需要过；没写就该一个都不出现）**、**§1.4 `attr_axis_lengths`：任务书点名了轴维度边界就必须声明、字段合法、且产得出来（产不出即 fail-closed，不许删字段绕）**…）。
...
- **任务书是验收权威**；PR 提供本轮 ABI、op_def 枚举、example 和目录等被测事实。任务书以“所有进入 AICore 的类型”定义集合时，op_def 负责枚举该集合成员；这不代表由 PR 覆盖任务书语义，更不代表验收通过。
- 抽完的 spec 交下游：`gen_cases.py`(Task1) / `run_workflow.py`(Task2/3)；或由 `op-acceptance` agent 继续编排 ③-⑥。
- **spec 的自检 ≠ 验收裁决**：本 skill 只产 spec 与 gaps，裁决唯一归确定性脚本链（`validator.py` + `perf_compare.py` + `validate_acceptance_state.py`），引用时逐字标来源、不自行宣告。
...
**详规见** `references/taskdoc-to-spec.md`（目标 schema · 字段映射 · **§1.2 dtype 冲突以任务书为准(C4)** · **§1.2a `dtype_deferred` 的两道门（能力来源硬校 + 终态不得干净 pass）** · **§1.3 torch 对标 / 多输出 / aclnn_py 形态怎么填** · verify_mode 决策树 · threshold 兜底 · 多算子拆分 · GPU 移植特例 · 自检清单）
