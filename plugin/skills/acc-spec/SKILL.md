...
1. **取材**（确定性活，下放给脚本）：
   ```
   python3 <plugin>/acc-common/fetch_source.py --taskdoc <路径|链接> --pr <PR链接> --out <workdir>
   ```
...
5. **落盘**：写 `<plugin>/acc-common/specs/<op>.spec.json`（或用户指定目录）。所有缺口/矛盾/推断落 `task_pr_gaps`，推断项标 `(推断)`。向用户复述：产了几份 spec、关键字段、gaps。
