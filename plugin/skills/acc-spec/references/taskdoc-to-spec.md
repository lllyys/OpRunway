...
| `op` / `execution.kernel_op_type` | `op` 来自任务书与公开 API；内部类型只来自 current `source_facts.derived.kernel_identity` 的唯一词法安全 `OP_ADD(<Identifier>)` 候选 | 两者是独立命名空间，允许不同。正式新 spec 写完整 `execution={kernel_op_type,source_binding}`，逐字复制 source fact；0/多候选、缺 binding、路径/SHA/摘要漂移均 fail-closed。禁止按算子名特判或从目录/API 猜内部类型 |
