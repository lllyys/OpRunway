...
| n | n | 整数 | 维度 |
| x_fill | x | 见下方 fill 词表 | x（inout）的数据填充方式 |
| incx | incx | 整数 | x 的步长 |
| y_fill | y | 见下方 fill 词表 | y（inout）的数据填充方式 |
| incy | incy | 整数 | y 的步长 |
| param0 | param | int 或 float | fixed_vector 第 0 个元素 |
| param1 | param | int 或 float | fixed_vector 第 1 个元素 |
| param2 | param | int 或 float | fixed_vector 第 2 个元素 |
| param3 | param | int 或 float | fixed_vector 第 3 个元素 |
| param4 | param | int 或 float | fixed_vector 第 4 个元素 |
| expect_result | 控制列 | 见下方状态词表 | 使用 csv_loader.h parseStatus 支持的全名 |
| nullX | x | 0, 1 | 1 时传 nullptr 且不分配该参数 |
| nullY | y | 0, 1 | 1 时传 nullptr 且不分配该参数 |
| nullParam | param | 0, 1 | 1 时传 nullptr 且不分配该参数 |
...
```text
case_name,description,n,x_fill,incx,y_fill,incy,param0,param1,param2,param3,param4,
expect_result,nullX,nullY,nullParam,random_seed
```
...
- `golden.kind`: `cblas`
- `golden.symbol`: `cblas_srotm`
- golden dtype: `float32`
- in-place 参数先快照原值再计算 golden：x, y
...
| PW | 28 | 确定性 pairwise 且通过 constraints/footprint |
| ED | 3 | 逐条 edge_cases.set 物化 |
| PF | 0 | perf.rows 与可选 sweep |

pairs 覆盖 196/196，infeasible 0 对，search_exhausted 0 对。
