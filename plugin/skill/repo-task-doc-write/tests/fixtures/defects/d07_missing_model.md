...
1. 要求在 FLOAT16 和 BFLOAT16 输入精度场景下，910B3 上的平均单次耗时（Avg time，单位 us）均不高于 GPU A100 标杆平均耗时的 1.25 倍（即达到标杆 0.8 倍性能）；

2. 性能自测case如下：

| B | H | D | Img_seq    | Text_len | Window_size                           | GPU A100 标杆（Avg time，us） | 910B3 判据（≤，us） |  |
|---|---|----|------------|----------|---------------------------------------|------------------------|------------------------|------------------------|
