...
|---|---|----|------------|----------|----------|-----------|-----|-----|
| q | 输入 | query tensor | tensor | FLOAT16、BFLOAT16 | BNSD | [B,N,S,D] <br> q、k、v、output 必须均为 4 维 BNSD 布局，shape 完全一致。B、N、S、D 均必须大于 0。S 必须不小于 image_seq_len + textLength，其中 image_seq_len = t * h * w。输入 tensor 不支持 broadcast，不支持 q/k/v 使用不同 shape 或不同 dtype。 | [-4, 4] | 触发参数校验报错 |
| k | 输入 | key tensor | tensor | FLOAT16、BFLOAT16 | BNSD  | [B,N,S,D] <br>约束和输入q一致| [-4, 4] | 触发参数校验报错 |
| v | 输入 | value tensor | tensor | FLOAT16、BFLOAT16 | BNSD | [B,N,S,D] <br>约束和输入q一致| [-4, 4] | 触发参数校验报错 |
| output | 输出(独立输出) | attention 结果 | tensor | FLOAT16、BFLOAT16 | BNSD | [B,N,S,D] <br>约束和输入q一致| [-4, 4]（softmax 加权 v 的凸组合，不超出 v 的值域） | 触发参数校验报错 |
| windowSize | 属性 | 每个 head 的 (t,h,w) 奇数窗口；长度为 1 时广播到所有 head，长度为 windowSizeLen（等于 head 数）时逐 head 生效 | list_list_int | int | - | 长度为 windowSizeLen 的 list，每项为 3 元组 (wt,wh,ww) | 每维 (wt,wh,ww) 均取 [1, 15] 内的奇数 | 触发参数校验报错 |
| windowSizeLen | 属性 | windowSize 数组长度，等于 windowSize 的长度 | scalar | int | - | - | [1, N]（N 为 head 数） | 触发参数校验报错 |
| textLength | 属性 | text token 数；hasText=false 时按 0 处理 | scalar | int | - | - | [0, S)（S 为 q/k/v 的 S 维长度）；hasText=false 时取 0 | 触发参数校验报错 |
| hasText | 属性 | 是否存在 text token | scalar | bool | - | - | True/False | 触发参数校验报错 |
| seqShape | 属性 | image token 三维尺寸，如 30x48x80 | string | - | - | - | 格式为 "axbxc"，a、b、c 均为 [1, 200] 内正整数 | 触发参数校验报错 |

### 2.5 算子实现约束

| 约束项 | 内容 |
| --- | --- |
| 非连续 Tensor 支持 | 不支持，q/k/v 必须连续 |
| broadcast 规则 | 不支持，q/k/v/output 的 shape 必须完全一致，不做维度广播 |
| dynamic shape 要求 | 不要求，B、N、S、D 在图编译期均为已知静态 shape |
| 原地与视图语义 | 不涉及原地语义，output 为独立输出，不复用 q/k/v 的内存，不返回视图 |
| 确定性计算要求 | 要求，同输入两次执行逐位一致 |
| 空 Tensor 与 0 维处理 | 不合法，B、N、S、D 任一为 0 时报错 |
