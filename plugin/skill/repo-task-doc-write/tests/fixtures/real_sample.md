...
**确定性计算要求**：算子需支持确定性计算，相同输入多次执行结果一致。
...
|---|---|----|------------|----------|----------|-----------|-----|-----|
| q | 输入 | query tensor | tensor | FLOAT16、BFLOAT16 | BNSD | [B,N,S,D] <br> q、k、v、output 必须均为 4 维 BNSD 布局，shape 完全一致。B、N、S、D 均必须大于 0。S 必须不小于 image_seq_len + text_length，其中 image_seq_len = t * h * w。输入 tensor 不支持 broadcast，不支持 q/k/v 使用不同 shape 或不同 dtype。 | (-∞,∞) | 触发参数校验报错 |
| k | 输入 | key tensor | tensor | FLOAT16、BFLOAT16 | BNSD  | [B,N,S,D] <br>约束和输入q一致|  (-∞,∞) | 触发参数校验报错 |
| v | 输入 | value tensor | tensor | FLOAT16、BFLOAT16 | BNSD | [B,N,S,D] <br>约束和输入q一致|  (-∞,∞) | 触发参数校验报错 |
| output | 输出(独立输出) | attention 结果 | tensor | FLOAT16、BFLOAT16 | BNSD | [B,N,S,D] <br>约束和输入q一致|  (-∞,∞) | - |
| window_size | 属性 | 每个 head 的 (t,h,w) 奇数窗口；长度为 1 时广播到所有 head，长度为 H 时逐 head 生效 | list_list_int | int | - | - | (0, ∞) | 触发参数校验报错 |
| text_length | 属性 | text token 数；has_text=false 时按 0 处理 | scalar | int | - | - |  (0, ∞) | 触发参数校验报错 |
| has_text | 属性 | 是否存在 text token | scalar | bool | - | - | True/False | 触发参数校验报错 |
| seq_shape | 属性 | image token 三维尺寸，如 30x48x80 | string | - | - | - | - | 触发参数校验报错 |
...
1. 要求在fp16和bf16输入精度场景下，基于910B3的性能与0.8倍【FastVideo.sliding_tile_attention接口 + GPU（A100）】持平；
...
本任务使用AscendOpTest工具（https://gitcode.com/HIT1920/AscendOpTest ）进行验收，请根据本任务给出的[自测用例和测试指导](测试用例文件夹路径)完成自测，并输出自测报告。
...
| k | (0, 1)正态分布，100% | - |
| v | (0, 1)正态分布，100% | - |
| window_size |- | [1, 100] 平均分布 |
| text_length | - | [1, 100] 平均分布 |
| has_text | - | True、False |
| seq_shape | - | "axbxc", a、b、c都在[1, 200] 平均分布 |
