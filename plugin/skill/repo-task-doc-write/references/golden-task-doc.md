...
本任务的自测用例的输入数据生成遵循如下分布规则，如果需要进行更完善的自测，请参考如下规则生成自测用例。S 是 q/k/v 的序列维，按 §2.1 由 seqShape 与 textLength 共同派生（S = textLength + T×H×W），不是独立采样的量，生成顺序必须按下表先定 seqShape、再定 textLength：
...
| hasText | - | True、False |
| seqShape | - | "axbxc"：a、b、c 各自在 [1, 200] 内均匀取整数，记 image_seq_len = a×b×c |
| textLength | - | hasText=true 时，在 [0, image_seq_len] 内均匀取值，再令 S = textLength + image_seq_len（S 由此派生，不独立采样）；hasText=false 时固定为 0，S = image_seq_len |
