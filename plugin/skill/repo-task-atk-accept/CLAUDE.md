...
本目录承接验收侧 S0 与 S3–S5。阶段归属的真相在
`references/artifact-contracts.json` 的 `stages[*].skill`，量具从那里读取：

- S0 接收封印交接包，核对环境、任务书、ATK 版本与工程接口
- S3 构建、安装、绑定并完成冒烟
- S4 执行精度与性能测试，由量具裁决
- S5 汇总结论、证据链与复现包

验收侧从 `repo-task-case-gen` 的只读原包复制工作副本，不回到 S2 重生成；工程公开接口面
可以读，kernel、host、tiling 与内部断言仍是待验收实现，不得用作判据。脚本归属由骨架
`scripts` 表与 `test_script_ownership.py` 锁住。

交接包是两侧唯一接口，规范在 `references/handoff.md`。运行时读本目录
`SKILL.md`，使用者说明在 `docs/skills/repo-task-atk-accept/`，改阶段先改骨架。
...
├── CLAUDE.md             # 本文件：验收侧开发规则
├── SKILL.md              # 验收侧入口：S0、S3–S5
├── references/           # 验收知识、交接规范与共用事实
│   ├── artifact-contracts.json  # 唯一骨架
│   ├── handoff-intake.md        # 交接包接收规则
│   ├── build-deploy.md          # 构建、安装与绑定
│   ├── reporting.md             # 精度裁决与报告
│   └── handoff.md               # 两侧交接面
├── scripts/              # 接收、构建取证、执行与裁决量具
└── tests/                # 验收侧、共用件与行文防漂移测试
```
...
`test_document_style.py` 的 `PROSE_BASELINE` 对现存文件合计，验收侧有
**87 处存量违规**：入口 `SKILL.md` 1 处，references 共 86 处。

欠账最重的四份 reference 是 `build-deploy.md` 23 处、`atk-cli.md` 16 处、
`reporting.md` 15 处、`builtin-baseline.md` 9 处。
...
- **使用者设计：** `docs/skills/repo-task-atk-accept/design.md`
- **使用者上手：** `docs/skills/repo-task-atk-accept/quickstart.md`
...
**最后更新：** 2026-08-24（S11：验收侧成为平级、自足的 skill 目录）
