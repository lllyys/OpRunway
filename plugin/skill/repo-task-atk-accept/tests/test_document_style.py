...
def relative_path(path):
    ...
    side = layout_side()
    if relative == "SKILL.md" and side is not None:
        return f"{side}/SKILL.md"
    return relative
...
def stale_prose_baselines(baseline, paths, registered_references):
    """返回过期基线；展开产物缺另一侧文件不算过期。"""
    entry_pages = {"SKILL.md", "case-gen/SKILL.md", "acceptance/SKILL.md"}
    stale = []
    for name, budget in baseline.items():
        path = paths.get(name)
        if path is None:
            if Path(name).name in registered_references or name in entry_pages:
                continue
...
class DocumentStyleTest(unittest.TestCase):
    ...
    def test_skill_source_contains_no_tracked_python_bytecode(self):
        ...
        skill_path = SKILL_ROOT.resolve().relative_to(git_root).as_posix()
        result = subprocess.run(
            ["git", "ls-files", skill_path, "*.pyc"],
            check=True,
            capture_output=True,
            text=True,
        )
            ...
            for number, line in prose_lines(path):
                for match in script_ref.finditer(line):
                    ...
                    if (scripts_dir / name).exists():
                        continue
                    spec = ownership.get(name) or {}
                    if side is not None and spec.get("skill") not in {side, "shared"}:
                        continue
    ...
    def test_precision_standard_keeps_the_threshold_table(self):
        ...
        text = reference_text(self, "experimental_standard.md")
    ...
    def test_api_fields_have_documented_semantics(self):
        ...
        text = reference_text(self, "yaml-schema.md")
    ...
    def test_c_signature_reverse_lookup_is_complete(self):
        ...
        """
        text = reference_text(self, "atk-parameter-capabilities.md")
    ...
    def test_three_registration_keys_are_told_apart(self):
        ...
        text = reference_text(self, "plugin-authoring.md")
    ...
    def test_case_output_path_is_documented(self):
        ...
        text = reference_text(self, "atk-case.md")
    ...
    def test_soc_declaration_gate_is_terminal(self):
        ...
        text = reference_text(self, "build-deploy.md")
    ...
    def test_prose_baseline_has_no_stale_entries(self):
        ...
        registered = set(contracts.get("references") or {})
        stale = stale_prose_baselines(PROSE_BASELINE, paths, registered)
        self.assertEqual([], stale)

    def test_missing_unregistered_baseline_entry_is_stale(self):
        stale = stale_prose_baselines(
            {"references/removed.md": 1}, {}, {"kept.md"})
        self.assertEqual(
            ["references/removed.md: 基线里有但文件不存在"], stale)
    ...
    def test_interface_facets_keep_once_only_generation_rule(self):
        require_nested_source(self, "生成侧一次生成与验收侧汇总的跨侧行文")
        skill = skill_text()
        design = reference_text(self, "case-design.md")
    ...
    def test_optional_attr_rule_forbids_empty_compound_group(self):
        plugin = reference_text(self, "plugin-authoring.md")
    ...
    def test_smoke_requires_report_and_candidate_provenance(self):
        execution = reference_text(self, "execution.md")
        cli = reference_text(self, "atk-cli.md")
    ...
    def test_acceptance_header_source_rules_are_self_contained(self):
        handoff = reference_text(self, "handoff-intake.md")
        acceptance = side_page_for(self, "acceptance").read_text(encoding="utf-8")
    ...
    def test_pyaclnn_flow_keeps_abi_and_soc_gates(self):
        execution = reference_text(self, "execution.md")
        cli = reference_text(self, "atk-cli.md")
        deploy = reference_text(self, "build-deploy.md")
    ...
    def test_adapter_guidance_requires_runtime_contract_and_semantic_smoke(self):
        skill = skill_text()
        plugin = reference_text(self, "plugin-authoring.md")
        capability = reference_text(self, "atk-parameter-capabilities.md")
        self.assertIn("运行时契约表", skill)
    ...
    def test_capability_probe_records_typed_optional_tensor_factory(self):
        probe = script_text(self, "probe_atk_capabilities.py")
    ...
    def test_runtime_report_path_is_documented(self):
        ...
        text = reference_text(self, "atk-cli.md")
    ...
    def test_unreliable_truth_gates_state_their_exit(self):
        ...
        cap = script_text(self, "_atk_capabilities.py")
        self.assertIn("probe_atk_capabilities.py 重探", cap)
        self.assertIn("atk_versions", cap)
        validate = script_text(self, "validate_cases.py")
    ...
    def test_unknown_operator_class_is_extensible_not_refused(self):
        ...
        strategy = script_text(self, "_coverage_strategy.py")
    ...
    def test_atk_signature_self_check_greps_the_install_tree(self):
        ...
        binding = script_text(self, "_opapi_binding.py")
