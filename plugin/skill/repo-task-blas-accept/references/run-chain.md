...
存在有效复测轮时，性能状态与逐例结果由折叠得出，`performance` 段增加 `retest` 键；
字段表、报告增量与条件输出（无复测痕迹时输出与现行一致）见
retest-protocol.md「verdict 与报告增量」。`<工作目录>/verdict_notes.md`
是 A2′ 人工备注的落点（自由格式）：每次 A5 重渲染时原样并入 `report/report.md` 的
`备注说明`，文件缺失按空处理。
