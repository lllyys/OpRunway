...
| A1 env 正确检出真机工具链（cmake/g++/build.sh/frame/header/CANN/msopprof/cblas） | 实测（A3，sger） |
| npu-smi 在容器内解析不出设备（记"未知"，非门禁）——探针脆弱，待改 | 实测（A3） |
| 精度 harness 真机通：build-skip→list→名字映射→gtest json→结果 JSON | 实测（A3，sger TC_L0 全 PASS） |
| 性能 msopprof 流水线真机通：单次采集 + gtest JSON 执行证据 + 解析 | 实测（A3；msprof 后端 sger 全链 + ctpmv 200 例，2026-09-14；2026-09-23 换 msopprof 后复测 ctpmv） |
| kernel_us 单次采样可复现：同例跨进程偏差 ≤3% | 实测（msopprof，同例 6 次 15.40–16.46 us） |
| 换后端是口径变更不是等价替换：msopprof 读数为 msprof 的 0.64–0.84 倍，绝对差恒定 8.5–11 us（重放绕开首次调用惩罚） | 实测（2026-09-23 同机双跑 TC_PF_1001/1002/1003：23.78→15.30、45.40→35.72、68.50→57.26 us） |
| 两种口径谁更接近 GPU 对照组无法判定，换后端不等于更准 | GPU 基线 `perf.meta` 六个键当前全是 `unspecified`（含 `warmup`） |
| msopprof 同样不透传被测程序失败；但工具自身参数或初始化失败会退非零（`--ai-core=on` 退 255） | 实测（2026-09-23，CANN 9.0.1） |
