...
**判定脑子不在这**：算子验收的 pass/fail 唯一归确定性脚本链（`validator.py` 精度 + `perf_compare.py` 性能 + `validate_acceptance_state.py` 三级门 → 门控后写 `acceptance.json`，ADR 0007；⚠ **验收裁决当前只出自 `--mode cpp_extension`**（`run_workflow._ACCEPTANCE_RUNNER_FORMS` 只含 `cpp_extension`，见 `AGENTS.md` §4）——`--mode new_example` / `--mode aclnn_py` 自 2026-08-06 **停止准入、连跑都跑不起来**（逃生阀已删，显式指定同样被拒）；mock 侧 C5 起同样只产标 NON-ACCEPTANCE 的 `dev_run_summary.json`）。本 agent **不自行判算子 pass/fail**；`verify_runner` 判的是「runner 自身可信 / 未过」这道 **runner 自证门**（逐元素比手算 golden），与算子验收裁决是两回事，别混。
...
调度进来先判范围。⚠ **引擎侧已比这张表泛化**（2026-07-23 批 6b 调研实读更正）：`run_on_npu.sh` 的目录 / vendor 后缀 / build 旗标**都已生成化**（`OPRUNWAY_OP_SRC` / `OPRUNWAY_VENDOR_SUFFIX` / `experimental/` 前缀→`--experimental`），**不再字面硬编码 `experimental/math/$OP` + `${VEN}_math`**。真正把通路锁在 ops-math/aclnn 的是**三块**：① `build.sh --pkg --ops` 家族命令（catlass 是 `scripts/build.sh <example> -DCATLASS_ARCH`，不同）② opp 自定义 vendor 布局（`.run` 安装包 + `vendors/<v>/op_api/`）③ aclnn 两段式链接（`-lcust_opapi`/`-lopapi`）。故**当前代码闭环 = ops-<族> 仓 · opp 安装型产物 · aclnn 两段式接口**。放宽计划见 `dev-doc/oprunway-batch6b-design.md`。据 `pr_facts.target_dir` + `spec` 判：

| 情形 | 处置 |
|---|---|
| **`spec.runner_form == "aclnn_py"`（torch 对标 · ctypes-aclnn runner form）**<br>⛔ **已停止准入（2026-08-06）**：不但产不出验收裁决，**连真机入口都没有了**（逃生阀已删，`AGENTS.md` §4），以下机制仅作历史保留；要正式验收须迁 `cpp_extension` | ✅ **放行、但本 agent 不产 runner**：此形态**无 per-op runner 源**（op 工程即 DUT，`aclnn_runtime` ctypes runner op-中立、从 header 推 arity）→ **不生成 `oprunway_<op>_runner.cpp`、跳过 per-op `verify_runner`**（无源可自检）。⚠ **跳过 `verify_runner` ≠ 免验证**：本形态改由 CP-C 的 **aclnn_py harness 真机信任门**接住（见 `skills/acceptance-workflow` CP-C），信任门未过 / 未留证一律停在 CP-C。scope gate 只校 **ops-<族>仓形态**（**仓根** `build.sh` + `<op_subdir>/op_host/` + **在 `<op_subdir>` 下（有界递归，含 `op_host/op_api/`）能找到** `aclnn_*.h`（剔 `*_impl.h`）；由 `aclnn_adapter.find_aclnn_project` 复核 + 逐段软链守卫）。⚠ **接口头落点不预设是哪一层**——PR6429 真实布局是 `<op_subdir>/op_host/op_api/aclnn_median.h`，`<op_subdir>/` 下**并没有** `op_api/`（2026-07-24 dogfood 实测订正；旧文写成 `<op_subdir>/op_api/` 是错的）。⚠ **不要求 per-op `build.sh`、不要求 `op_graph/`**——2026-07-24 实测坐实 ops-nn 实验算子（PR6429 median）二者皆无、build 走**仓根** `build.sh --pkg --experimental --ops=<op>`（见 `dev-doc/oprunway-torch-baseline-design.md` §9.4/§9.6）；缺件 / 非标准两段式 / 有 opaque descriptor → ⛔ **BLOCKED**「不支持的接口能力」（域内假设：无状态 / 标准 aclnn 两段式 / 无 opaque descriptor）。过 gate → 回「无 runner 源、op 工程即 DUT、**待 CP-C harness 真机信任门**（过了才进 CP-D `--mode aclnn_py`）」摘要，**不硬塞、不宣称可直接进 CP-D**。dtype 白名单据 form 放开 int/bf16（`repo_adapter.supported_np("aclnn_py")`），故本表下方「dtype 超范围 → BLOCKED」那行**不适用 aclnn_py** |
| **`spec.runner_form == "cpp_extension"`（任务书对标 torch）**<br>✅ **当前唯一能产验收裁决的准入形态，也是 spec 未声明 `runner_form` 时的缺省** | ✅ **放行、但本 agent 不手写 per-op runner**：调用 `cpp_extension_codegen.py` 生成官方 `NpuExtension + npu_cpp_extension.h + EXEC_NPU_CMD_EXT` bundle；逐 case 调用只消费 gen_cases 已解析的 `aclnn_call`。真机 build/load/执行由显式 driver 完成并回传专属内容寻址收据；不复制旧 helper，不修改/重编 op-plugin，不复用 aclnn_py ctypes receipt。 |
| `experimental/math/<op>`（is_close/sign/equal 类）+ dtype ∈ {float32, float16}（**仅 `runner_form == "cpp"`**；⛔ 该形态 2026-08-06 起停止准入、无真机入口，本行仅作历史保留） | ✅ 在范围内，进 `gen_runner` |
...
| **换构建体系**（catlass 的 `scripts/build.sh <example>`）/ **换接口形态**（非 aclnn 两段式）/ 双实现 | ⛔ 返回 **BLOCKED**、记 gap、**转 P3**（按 `dev-doc/oprunway-batch6b-design.md` 扩「构建策略 + 接口分派」再来；⚠ **别再指 `OPRUNWAY_TARGET_DIR`**——它是幽灵变量，runner 通路的 `.sh`/`.py` 里根本没有，2026-07-23 更正），**不假装能选路径** |
...
- **不自行判算子 pass/fail**：算子验收裁决唯一归确定性脚本链（validator + perf_compare + validate_acceptance_state → acceptance.json，ADR 0007；**仅真机通路且仅 `cpp_extension` 准入**，`cpp` / `aclnn_py` / mock 都不产此件）；本 agent 只产**代码工件**（`golden.py` / `runner.cpp`）+ runner 自证结论，绝不新增自行宣告算子 pass/fail 的文本，引用产物裁决时逐字标来源。
...
- 下游：`gen_golden` → primary 在 CP-B 跑 `gen_cases.py --dry-run`；CP-C 自证门过（`cpp_extension` 形态 = build/load/vendor receipt 齐备且绑定来源锚；`cpp` 形态 = runner 过 `verify_runner`；`aclnn_py` 形态 = harness 真机信任门过且留证）→ 由 `acc-verify-rootcause:run_npu` 在 CP-D 走 `run_workflow.py --mode <mode> --source-facts <CP-A 取材目录>/source_facts.json`（⚠ `--source-facts` 在验收通路上**必给、缺席直接拒跑**；`<mode>` 据 `spec.runner_form` 派生，受控词表 `{cpp, aclnn_py, cpp_extension}`、**缺省 `cpp_extension`**：`cpp_extension`（或未声明）→ `cpp_extension`（✅ **唯一产验收裁决的通路**）、`cpp` / `aclnn_py` ⛔ **已停止准入**：派不出 mode、显式指定也被拒；`mock` / `catlass` / `catlass_mock` 派生不出、须显式指定。Task2 精度 + Task3 性能 + 三级门一次成）。
