...
- **触发**：CP-B primary inline 跑 `gen_cases.py <spec> --dry-run`（**plan-only**：不算 golden、不 import torch、不落 `.npy`、**不产任何裁决**）后，**报错或计划账本异常**（预算区间不合理、重点 dtype 未覆盖、特殊场景缺失、`case_id` 撞…），orchestrator 判为「疑 spec 侧问题」→ 带该报错再 dispatch 本 agent 的 `refine_spec`。
