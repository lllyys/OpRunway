...
- **干什么**：加载 `acc-spec` skill，按 `references/taskdoc-to-spec.md` 字段映射表逐字段抽，重点守住这几个最易错点（都在 ref 里）：
  1. **dtype 全集 vs 子集**：任务书显式枚举时逐字采用；任务书把范围定义为“所有进入 AICore 的数据类型”等实现域集合时，从本轮同一 PR head 的 `op_def` 与目标硬件配置枚举成员。不得复用旧 spec/报告，也不得因任务书未重复列举就回 `needs_user`。只有本轮任务书语义和本轮 PR/op_def 都无法确定集合时才问用户。再以确定性生成/runner 能力表求当前可测子集，写入 `params.dtype`，其余逐项挂账。
     ...
     **`dtype_deferred` 同样不是免检牌**（详规 ref §1.2a）：它必须**声明能力来源**——`capability_source` ∈ `{generation, runner, compute}`，指名是 `gen_cases._NATIVE` / `repo_adapter.SUPPORTED_NP_BY_FORM[<runner_form>]`（此来源另须写 `runner_form`）/ `precision_policy.SUPPORTED_COMPUTE_DTYPES` 哪一张表不支持；门拿**活表**交叉核验，🔴 **自报不支持而表里其实支持 → 拒该 gap**（该 dtype 随即按「静默收窄」判 BLOCKED）。且挂了它的 dtype 若一条用例都没跑，终态**不会**是干净 `pass`（至少 `needs_review`）。所以别用它罩「不想测」的 dtype——写上去只会把结论从 pass 变成 BLOCKED/needs_review。
  ...
  4. **runner 锚定线索**：先读 `source_facts.payload.derived.api_mapping`，只接受 `status="exact"`；按任务书出现顺序消费完整 `aclnn_entries` / `targets`，逐 API 只认唯一 public header + 唯一 example。**禁止从 `key_files` 排序后挑第一个 example**；任务书点名多 API 时一个不少，0 命中/多解均回 CP-A，不靠算子名或路径猜。输入 dtype 再从这些已定身份的 example 与 op_def 交叉。
  ...
  9. **附带自测只作 reference 的显式裁定**（N5）：dispatch/`confirmed_constraints` 若明示「正式 caseset/golden 由 OpRunway 生成」，则必须写 `precision.case_source="generated"` + `precision.reference_case_material_role="reference_only"`。这项已确认约束优先于「任务书有 self_test 链接」的旧启发式；链接/源仓 selftest 可作 requirement/coverage reference，但不得决定 `case_target`、case 身份或 golden，也不得写 `precision.taskdoc_caseset`。未有该显式裁定时才走 ref §1.6 历史两档。
...
受控词表仍保留 `cpp` / `aclnn_py`，是为了读懂旧 spec 与按旧 form 解释能力账本，并不表示本 agent
还能生成它们。2026-08-06 以前，`cpp` 使用 per-op runner，真机 dtype 白名单只有
fp32/fp16/bf16，`int32` 落 `DEFERRED_NP_BY_FORM["cpp"]`；`aclnn_py` 使用通用 ctypes 两段式
调用桥。这两条旧形态现在都没有真机入口，历史结果也不能替代本轮 form 与 provenance。
