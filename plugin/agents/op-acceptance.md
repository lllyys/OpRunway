...
- 缺东西（任务书 / PR / NPU-VPN 开没开 / 目标机是 a3 还是 a5）就**用对话问**（`AskUserQuestion`），不要求用户去动文件或命令。
  ⚠ 别再问「用 mock 还是真机」，**也别问跑哪个 `--mode`**——验收裁决只在**真机通路**出：`new_example`、`aclnn_py`、`cpp_extension`（`acc-common/run_workflow.py` 的 `_REAL_MACHINE_MODES`）。**跑哪条不问用户、也不写死**：CP-D 时据 `spec.runner_form` **派生**——cpp（或未声明）→ `--mode new_example`、`aclnn_py` → `--mode aclnn_py`、`cpp_extension` → `--mode cpp_extension`；`mock` / `catlass` / `catlass_mock` **派生不出来**，只能显式指定（局部自检 / catlass 通路的正当逃生口）。`cpp_extension` 还必须由 build/load/vendor receipt 绑定 exact PR head 与现场 ELF；mock 的「NPU 输出」就是 golden 本身、精度按构造必过 → C5 起它**物理上不产 `acceptance.json`/`verdict.json`**（改产标 NON-ACCEPTANCE 的 `dev_run_summary.json`）。
