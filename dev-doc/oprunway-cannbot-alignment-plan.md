...
- **我们现状（2026-07-26 真机复核后）**：采集层仍是 `aclnn_runtime/perf_msprof.py`，判定层仍是 `perf_compare.py`。custom 与 torch baseline 已统一为 **msprof CLI + `libms_tools_ext.so` ctypes MSTX + CSV task_time**；warmup=5/repeat=20、kernel-only、median×launches、setup 剔除、多 kernel 求和、行为五分类及双边采集配置一致性闸均保留。`torch_npu._C._mstx` 在 msprof 下 rid=0 的旧 finding 只说明该 Python API 不可用；按 cannbot 调 `mstxRangeStartA` 的真机 probe 得到 `range_id=1` 且可裁出有效 kernel 窗，因此不再需要 baseline 走 torch profiler DB。报告已具备 `by_dtype`、`overall_speedup`、`cases_above_threshold/cases_scored`。`numel<4096 → trivial-met` 已从 `perf_compare.py`、`gpu_baseline.py` 和验收门整体移除，所有性能 case 均须真实双边采集；缺基线就 blocked。
- **gaps**：
...
- **我方证据**：`plugin/acc-common/aclnn_runtime/perf_msprof.py` 的 `DEFAULT_WARMUP/DEFAULT_REPEAT`、`TIMING_SCOPE`、`KERNEL_ACCOUNTING`、`MSPROF_EXTRA_ARGS`、`collector_for/measure_side`；`plugin/acc-common/perf_compare.py` 的真实逐例 ratio 与报告聚合；`plugin/acc-common/gpu_baseline.py` 的全性能 case 集合门；2026-07-26 A3 full-50 重算：50 个性能 case、48 个有效双边 ratio、2 个 torch BF16 dim=1 baseline blocked、`trivial_rows=0`，其中原 numel<4096 的 24 个 case 为 22 个真实评分 + 2 个明确 blocked。
