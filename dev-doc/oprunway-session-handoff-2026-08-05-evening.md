...
> 🔴 **本文已不是当前交接入口。** 2026-08-06 的合并把「本地来源」这条线换成了另一套实现
> （`source_provenance.py` + `local_snapshot`，见 `AGENTS.md` §9.3），本文描述的 `dut_source` /
> `local_checkout` / `root_digest` 那套**代码已删**。合并后**尚无单一权威 handoff**：
> 本文与 `dev-doc/oprunway-session-handoff-2026-08-05.md` 两份并列、都成文于合并之前
> （`AGENTS.md` §5.6）。下一轮开工第一件事是产一份合并后的新 handoff。
>
> **合并翻掉了本文的四条结论，读下面正文时按这张表打折：**
>
> | 本文原话 | 现状 |
> |---|---|
> | §3「`aclnn_py` + `local_checkout` 结构性 fail-closed，构建端根本不存在可对账的锚，不是排期问题」 | ❌ **已被证伪。** 保留的那套在容器内内联同一份摘要算法算出 `SNAPSHOT_SHA256` / `SUBTREE_SHA256` 与取材侧对账，锚是存在的；那道门已放开 |
> | §3 / §5「`root_digest` 只覆盖 `op_subdir`，不含仓级构建脚本」 | ⚠ **已缓解。** 现行收据同时绑整树与子树 merkle；但跨端对账用的仍是子树那一个 |
> | §3「三级门的残留伪装面（`source_facts` 缺席时查不出伪装）」 | ✅ **已封死**：验收通路上 `--source-facts` 必填、缺席即拒跑 |
> | §5「`_LOGIC_FILES` 加了 `dut_source.py`」 | 换成了 `source_provenance.py`（那个模块没了）。**结论不变**：旧的 aclnn 信任门收据仍会 revalidate 失败，仍是正确行为 |
>
> 仍然成立的：`runner_form` 收敛到 `cpp_extension`（§2 Step 6）、CP-F 对非准入通路一律拒跑、
> 「本地来源的性能维未见证」、以及 §5 里与来源通路无关的那几条坑。

> 只写「接下来做什么、从哪开始、有什么坑」，不堆历史。流水看 `dev-doc/oprunway-changes-brief.md`。

---

## 1 · 一句话

`dev-doc/oprunway-local-source-plan.md` 的 Step 0–6 **全部落地**，本地 checkout 已是一等被测来源通路，
...
细节见 `dev-doc/oprunway-local-source-realmachine-validation.md`。
...
4. workflow 治理批：`dev-doc/oprunway-workflow-governance-plan.md`，按其 §6 分批。
