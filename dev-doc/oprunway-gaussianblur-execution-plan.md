...
> 表格里的**状态与数值**取自 2026-08-03 真机实跑的产物；夹叙的分析、口径解释与流程评价是**判断**，
> 凡属推断处已就地标注（如 §3 的 4.6 精度阈值来自工具缺省，标了「推断口径」）。
>
...
本轮用例含 3×3 与 5×5 两种**可分离**卷积；非有限值会沿行、列两趟扩散到整个卷积邻域。
任务书既未规定 NaN/Inf 语义，这三条**虽按当前 golden 对拍为 pass，也不能据此形成任务书层面的功能结论**。
...
**降级点**：`pr_head_unbound` —— 在当前可访问的上游 `cann/ops-cv` 范围内未检出该 PR
...
| 2 | ~~性能 baseline 用哪个~~             | **已按 AGENTS.md 5.10 定案**：默认不做 GPU 对比；任务书的 `0.45×` 条款按「未验收」进 `task_pr_gaps`。仅当你明确要求做 GPU 对比时才需拍板 |
| 3 | **性能口径**（warmup / repeat / 统计量） | 计时范围已按 5.10 定为 msprof kernel-only；但任务书对 warmup、repeat、统计量只字未提，这部分仍缺                             |
| 4 | **交付件缺口怎么处理**                   | PR 未交付 OpenCV C\++ 适配层（§任务概述/§1/§7 均标必选）。停跑，还是部分执行但不得 PASS                                      |
| 5 | **NaN/Inf 期望行为**                | 任务书未规定，而工具会强制铺这三条 case                                                                          |

第 1、3 条 CP-B0 已机读判出并阻断（`NEEDS_USER`）；第 2 条按 5.10 已定案。第 4、5 条目前**只在 spec 的 `task_pr_gaps` 里、
靠人手写**——这正是下一节要说的问题。
...
| 3 | ~~「任务书必选交付件 ↔ PR 实际交付物」无任何机器对账~~ | **本轮执行时**如此：CP-B0 只读任务书（刻意设计）、`fetch_source` 只读 PR，两者从不碰面，判断全靠 agent 手写 `task_pr_gaps`。**现已由 `reconcile_deliverables.py` 落地**；本轮产物若要按新门禁采信，须重新对账并生成新收据 |
| 4 | ~~AGENTS.md 5.10 只有文字没有代码~~      | **本轮执行时** `perf.mode="measure_only"` 未实现，故性能维只能挂起。**现已实现全链**；须按新口径重新生成 GaussianBlur 的性能证据与验收产物                                                         |
