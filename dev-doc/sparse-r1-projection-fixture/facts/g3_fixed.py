...
FACTS = {'schema_version': 1,
 ...
 'params': [{'name': 'handle', 'ctype': 'aclblasHandle_t', 'role': 'handle'},
            ...
            {'name': 'pv',
             ...
             'dir': 'in',
             'len': 3,
             'samples': [[1.0, 2.0, 3.0], [0.5, -1.0, 0.25]]},
            {'name': 'outv',
             'ctype': 'float*',
             'role': 'fixed_vector',
             'dtype': 'float32',
             'dir': 'out',
             'len': 2},
            {'name': 'ipiv', 'ctype': 'int*', 'role': 'int_array', 'dir': 'out', 'len': 'n'},
            {'name': 'ib',
             'ctype': 'long*',
             'role': 'int_array',
             'dtype': 'int64',
             'dir': 'inout',
             'len': 'n',
             'producer': 'lapacke_sgetrf(ipiv)'}],
 'constraints': ['n >= 1'],
 'golden': {'kind': 'lapacke', 'symbol': 'LAPACKE_sgetrf'},
 'verify': ['index_exact', 'pivot_validity'],
 'edge_cases': [{'name': 'pin_pv0',
                 'set': {'pv0': 9.5},
                 'expect': 'ACLBLAS_STATUS_SUCCESS',
                 'source': '合成 fixture：fixed_vector 元素键（首元素）'},
                {'name': 'pin_pv2',
                 'set': {'pv2': -4},
                 'expect': 'ACLBLAS_STATUS_SUCCESS',
                 'source': '合成 fixture：fixed_vector 元素键（末元素，int 值）'}],
 'sources': {'params': '合成 fixture：sparse-r1-projection-matrix.md §D 组3（fixed_vector/int_array 组）'}}
# ===== FACTS 区结束 =====
