...
FACTS = {'schema_version': 1,
 ...
 'params': [{'name': 'handle', 'ctype': 'aclblasHandle_t', 'role': 'handle'},
            ...
            {'name': 'x',
             ...
             'inc': 1}],
 'constraints': ['n >= 1'],
 'golden': {'kind': 'loop', 'symbol': 'ref_g6_trap3'},
 'verify': ['full'],
 'dtype_profiles': [{'name': 'c64',
                     'assign': {'ct': 'COMPUTE_32F'},
                     'scalar_dtype': 'complex64',
                     'golden_dtype': 'complex64',
                     'precision_row': 'FLOAT32'}],
 'edge_cases': [{'name': 'set_alpha',
                 'set': {'alpha': 2.0},
                 'expect': 'ACLBLAS_STATUS_SUCCESS',
                 'source': '合成负例：dtype_from 标量在复数 profile 下只收裸数字，物化时二元解包炸'}],
 'sources': {'params': '合成负例：蓝图 §C 陷阱3——复数 profile 下 edge set dtype_from 标量'}}
# ===== FACTS 区结束 =====
