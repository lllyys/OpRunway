<!-- 本文是 2026-08-06 合并的产物：同一天、同一文件名下原本有两份互不相干的交接
     （主线 GaussianBlur / ops-cv 那条，与 roll19 本地来源那条）。两份都保留，
     主线那份作正文，另一条作附录 A。 -->

> 🔴 **合并后尚无单一权威 handoff。** 本文的正文与附录 A 都成文于 2026-08-06 的合并**之前**，
> 各自只描述自己那条线，**都不描述合并后的状态**。同日还有第三份
> `dev-doc/oprunway-session-handoff-2026-08-05-evening.md`（roll19 那条线的晚间版，
> 里面有四条结论已被合并翻掉，那份自带更正表）。
> 下一轮开工第一件事是产一份合并后的新 handoff，届时这三份一起降为历史材料（`AGENTS.md` §5.6）。
...
# 附录 A · 同日另一条工作线的交接（roll19 / 本地来源）

> ⚠ **本附录整体是历史材料。** 它描述的 roll19 工作线里，「本地来源」那套实现
> （`dut_source` / `local_checkout` / `root_digest`）已于 2026-08-06 合并时被取代、代码已删；
> 现行口径见 `AGENTS.md` §9.3。仍然成立的是 `runner_form` 收敛到 `cpp_extension`、
> 以及 §A.4 那三条判断。附录里说的「当前入口」指的是那条线自己的晚间交接，不是本文。

> 原文自带的说明：本附录记录的是 08-05 白天那一段，文中所有「本轮」「接下来做」都指**那一段**。
> 它当时指向的「当前入口」是同线的 `dev-doc/oprunway-session-handoff-2026-08-05-evening.md`。
> 历史流水看 `dev-doc/oprunway-changes-brief.md`。
...
**这一段**只产文档，未改任何 `plugin/` 代码；方案的落地发生在之后的几段，
状态以 `dev-doc/oprunway-changes-brief.md` 顶部与晚间交接为准。
...
| 步 | 做什么 | 文档 |
|---|---|---|
| **1** | 本地来源通路 + `runner_form` 收敛到 `cpp_extension` | `dev-doc/oprunway-local-source-plan.md` |
| **2** | 修两个脚本 bug（小改，不修会绊住第 3 步的测试） | `dev-doc/oprunway-roll-complex64-trial-findings.md` §2 **A1 / A2** |
| **3** | workflow 治理批（确认门 / 执行路径 / 耗时监控 / 数据确定性 / 轴集 / 样例隔离） | `dev-doc/oprunway-workflow-governance-plan.md`，按其 §6 分批 |

想知道**为什么**要做这些：`dev-doc/oprunway-roll-complex64-trial-findings.md`（问题清单，非 plan）。
