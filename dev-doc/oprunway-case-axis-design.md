...
> **本节只收敛 §1 的 6 个决定，不回写 §1~§12。** 判定依据是 §12 的实测，以及步骤 11b
> （提交 `1d2e57c`）与 roll19 后续已经落地的代码事实。分类含义沿用本轮评审口径：
> **(a)** 已被后续工作实际解掉；**(b)** §12 证据足够给出可执行结论；
> **(c)** 证据不足或属于价值取舍，必须留给用户。

| # | 定性 | 收敛结论 |
|---|---|---|
| ① | **(a)+(b) 方向已实现，档值已有派生默认但未拍板** | **先补 shape 轴的结构覆盖，再谈扩值域等新轴。** `torch_parity` 已按接口能力去退化：检测到 `attribute_profiles` 含 `axis_class`，才把实际可能被选中的轴提升到 ≥2；无轴选择器的 elementwise 仍为 `(L,1,…,1)`，没有按算子身份分派。现派生档值是工程可用的**默认**，不是价值判断的最终结论；用户仍可因 numel/rank 解耦、机时或边界定位目标选择别的档值。默认形状与配置状态见 §13.1/§13.3。 |
| ② | **(a) 后续实现已实际解掉** | `torch_parity` 对三类受控 `operator_class` 均保持特殊场景 0 条；`special_scenario_policy` 把“零实测支撑下不扩面”的 `reason + evidence + emitted=0` 与 `case_target` 关系落账。`allow_empty_tensor` / `empty_axis` 等 legacy 键在本档仍是声明即拒；未来若有实测证据重开，特殊场景也只能独立叠加、不进笛卡尔。 |
| ③ | **(b) §12 足够支持“不做”** | **暂不引入值域 regime 轴。** 当前 `torch_parity` 只有 `uniform` 一档，结构上没有第二个 regime 可对照；因此“值域 × dtype 有交互”的收益至今是**零实测支撑**。便宜不等于有证据，不能仅因某类算子不涨例数就把轴加进去。⚠ **这是有意不加，不是遗漏**：重新提出引入前，必须先有一份至少含两种 regime、能在其它轴相同条件下比较或交叉的矩阵，并用它产出实测交互证据；在那之前保持 `uniform` 单档。 |
| ④ | **(a) 步骤 11b 已实际解掉** | 提交 `1d2e57c` 已把三重记账落在原判据处：`plugin/acc-common/gen_cases.py:_torch_parity_excluded` 负责把每条带 `combo + reason + evidence` 的部分排除展开为完整组合并拒绝未知值、重叠与无证据缩水；`plugin/acc-common/gen_cases.py:_torch_parity_plan` 逐字校验 `full_cartesian − excluded_total == expected == case_target == emitted`，并把轴取值、排除、三个数及不同/重复组合数写入 `case_matrix_ledger`。`case_target` 仍必填、无缺省；`case_target_source` 仍明确只是无消费方的散文，不能冒充机器门。 |
| ⑤ | **(b) §12 实测证据足够判定** | **重复组合保留，并显式记账；不折叠。** §12.9 已逐例确认有 6 组同组合额外随机样本出现 pass/fail 分裂；随机只留一个代表会漏掉 0~6 个 fail 组合，均值 2.82。保留相对当前 caseset 的覆盖代价为零，而折叠会真实丢信号。当前实现已与本决定同向：`plugin/acc-common/gen_cases.py:_torch_parity_combination_stats` 按 `(dtype, 实际 shape, 解析后 attrs)` 统计，`plugin/acc-common/gen_cases.py:_torch_parity_plan` 在 `case_matrix_ledger` 和 `coverage_strength` 中分别写 `distinct_combinations` / `duplicate_cases`；报告必须说“1344 例、1200 个不同覆盖组合”，不得把 emitted 当 distinct。 |
| ⑥ | **(a) 步骤 11b 已实际解掉** | 提交 `1d2e57c` 已删除 Median `torch_parity` spec 中不被消费的 `attr_matrix`，attr 轴只由 `precision.torch_parity_matrix.attribute_profiles` 声明；`plugin/acc-common/gen_cases.py:_TORCH_PARITY_UNCONSUMED_KEYS` 把 `attr_matrix` 列为本档禁用键，`plugin/acc-common/gen_cases.py:_reject_unconsumed_legacy_keys` 对再次声明 fail-closed。现在不是“靠样例刚好只写一处”，而是通用生成入口会阻止第二处重新长回来。 |

### 13.1 仍须用户拍板的边界

只剩一处不能被本节越权压平：

1. **决定是否用不同档值替换现派生默认。** 最低不变式已经由 §12 判定：必须取到“长归约轴且 batch > 1”，
   且不能只有均分档；代码已据此给出下表的派生默认并通过零跑测硬门，但具体档值仍会在
   **numel/rank 解耦、用例机时、边界定位能力**之间取舍。默认解决“现在能按什么规则生成”，没有替用户回答
   “哪组值最值得长期保留”。若要改，先给候选布局表和期望预算，再在真机重跑完整新矩阵；不能拿历史基线代替。

   令 `L = shape_profiles[].leading_dim`。当矩阵同时声明 first/middle/last 三类 `axis_class` 时，当前默认是：
   ...
   因而 large/rank4 的默认确为 `(262144,2,1,2)`。若 profile 只含部分类别，则只提升那些
   **实际可能被选中的轴**；没有 `axis_class` 时仍用 `(L,1,…,1)`。

### 13.2 收敛后的执行边界

- 已可直接执行：④ 的三重记账与 ⑥ 的单一声明保持现状；⑤ 保持“留 + 记账”；③ 不加值域 regime。
- ① 的现派生默认已可执行：只在 `torch_parity` 档生效；`legacy` 生成路径与 4 个由 sha256 钉死的历史
  caseset 字节不动。
- ② 已明确为“有意不产”并落机器账本；不是从未考虑，也不得把 0 条写成已经覆盖了特殊场景。
- 零跑测、复算现有 caseset 的硬门结果是：平凡归约 `624（46.4%）→0`；
  `浮点 ∧ 归约长度>24576 ∧ batch=1` 从 `90→42`；
  `浮点 ∧ 归约长度>24576 ∧ batch>1` 从 `0→48`。
- ⚠ 这道硬门只证明历史缺陷触发区域**没有被结构性删光**，不证明历史 58 条失败会在新矩阵中逐条复现。
  caseset 已变，必须重新真机验收；把“触发区域仍有覆盖”写成“58 条会复现”会把证据读反。
- §1 六项主决定现均已有可执行状态；① 的**具体档值仍只是可替换默认**，不能因此写成价值取舍已拍板。

### 13.3 已认账的代价、基线与配置状态

- **numel 代价：**numel 不再与 rank 完全解耦；当 first/middle/last 都出现时，rank≥3 的默认最多为旧档的
  **4 倍**，large 档也一样。默认值是覆盖起点，不是“4 倍一定值得”的拍板。
- **字节代价：**`torch_parity` caseset 字节已经改变；这不是可与旧产物做逐 case 对照的同一矩阵。
- **历史基线作废：**Median 的 1344/58（仓内样例 spec、本地来源）与 1152/51（当轮 per-run spec、PR 来源）
  两组历史真机基线对这份新矩阵均不可再作回归对照。下次若直接比较，会把“矩阵换了”误判成实现回归；
  两组数的历史效力不被改写，但都不能替代新矩阵的重跑结果。
- **当前配置状态：**`shape_profiles[].leading_dim` 可配；具体布局仍由
  `plugin/acc-common/gen_cases.py:_torch_parity_shape_layout` / `_torch_parity_shape` 派生，
  `spec_schema_template.jsonc` 暂不暴露一个无人消费的布局字段。要把布局档值开放给 spec，接入点必须同时覆盖：
  `_torch_parity_plan` 对 `shape_profiles` 的严格键/值校验、布局受控词表、上述两个函数的实际消费，以及
  caseset/dry-run 账本。词表外值、显式坏值和声明/能力冲突都须 fail-closed；这些门齐备前，用户应改代码点而不是
  往 spec 塞一个会被未知字段门拒绝的键。
