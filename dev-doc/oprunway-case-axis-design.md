...
> （提交 `1d2e57c`）已经落地的代码事实。分类含义沿用本轮评审口径：
> **(a)** 已被后续工作实际解掉；**(b)** §12 证据足够给出可执行结论；
> **(c)** 证据不足或属于价值取舍，必须留给用户。

| # | 定性 | 收敛结论 |
|---|---|---|
| ① | **(b) §12 实测证据足够判定顺序** | **先补 shape 轴的结构覆盖，再谈扩值域等新轴。** 驱动力是 §12.12 的硬覆盖缺口，不是单纯追求少跑平凡归约。实现不得把现 shape 只换成 `balanced`：§12.11 表明这种改法会把已知缺陷的暴露面从多个 rank 压回 rank 1。shape 契约至少须能构造「长归约轴且 batch > 1」的 `(B, K)` 类形态，同时保留长轴压力；判据按接口能力（轴选择器 / 多维布局语义）触发，绝不按算子名触发。**本条决定的是先后顺序与最低覆盖不变式，不替用户填写具体布局档数值**；剩余取舍见下文。 |
| ② | **(c) 价值取舍，留给用户** | `torch_parity` 是否产特殊场景仍不定。§12 对空、标量、上下边界没有任何输入，无法在「忠实参考仓、structural 类不产特殊场景」与「沿用本仓 legacy 的边界底线」之间判优；这不是再分析一次 fail 表就能解的技术问题。 |
...
1. **决定 ② 的规则优先级。** 用户须在两条政策中选一条：
   - 参考仓忠实优先：structural 类保持零特殊场景；代价是空、标量、上下边界不由
     `torch_parity` 覆盖，相应 legacy 开关继续拒绝声明；
   - 本仓边界底线优先：独立叠加边界场景；代价是偏离参考仓、caseset 字节与数量变化，且须明确
     各场景的合法性与 golden 语义。

   缺的不是再跑一次现矩阵，而是**用户对“参考仓忠实度”与“本仓最低边界覆盖”谁优先的价值选择**。
   若希望先用数据估价，至少要补一份带这些特殊场景的候选矩阵并真跑；它只能量化收益与成本，仍不能替用户决定政策优先级。

2. **决定 ① 下的具体布局档取值。** 最低不变式已经由 §12 判定：必须取到“长归约轴且 batch > 1”，
   且不能只有均分档；但具体保留几档、是否把现 `leading_heavy` 作为独立基线档、长轴取值放在切分边界哪一侧，
   仍会在**历史 caseset 可比性、用例数量、边界定位能力**之间取舍。现数据只能把切分阈值夹在
   `(2047, 262144]`，不足以替用户填数。拍板前至少还缺：候选布局表、期望的 caseset 预算/基线连续性选择，
   以及一轮覆盖“长归约且 batch > 1”候选格子的真机跑测，用来核实际耗时和缺陷暴露面。

### 13.2 收敛后的执行边界

- 已可直接执行：④ 的三重记账与 ⑥ 的单一声明保持现状；⑤ 保持“留 + 记账”；③ 不加值域 regime。
- 下一项设计工作：先按 ① 的最低不变式拟 shape 候选布局，但**在 13.1(2) 拍板前不写死具体档值**。
- 唯一仍悬而未决的 §1 主决定是 ②；不得把“没有特殊场景”静默解释成已拍板。
