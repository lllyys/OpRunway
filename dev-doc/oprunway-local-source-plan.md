...
> 🔴 **目标 1（本地来源）的这套实现已于 2026-08-06 合并时被取代、代码已删。**
> 本方案落地的是 `dut_source.py` 判别式 + `local_checkout` / `root_digest` /
> `oprunway.local_subtree_merkle` v1。合并保留的是另一套等价能力：`source_provenance.py` 的
> `declared_source_form × provenance_kind` 两轴词表 + 快照 merkle + `--pr-snapshot` / `--target-dir`
> （现行口径见 `AGENTS.md` §9.3）。**本方案 §2–§7 里的字段名、CLI 参数与文件清单不再对应仓里的代码。**
>
> **目标 2（`runner_form` 收敛到 `cpp_extension`）仍然有效、代码仍在**（`AGENTS.md` §4）。
>
> 本文继续留档的价值有两处：① 目标 2 的论证与落地记录；
> ② 目标 1 那一轮逼出来的**边界处理清单**（`completeness=blocked` 非 0 退出、遍历错误不得静默吞、
> 取材期改动重校、摘要策略机器可核、反向排他、`--source-facts` 必填），
> 这些**已逐条移植进保留的那套骨架**——移植清单见 `dev-doc/oprunway-changes-brief.md` 顶部那条合并记录。
...
**背景**：`dev-doc/oprunway-roll-complex64-trial-findings.md` §1.3 / §2 B1 / §2 B2 / §6 目标 2 / §6 目标 8
**范围**：只做来源接入 + 通路收敛。**不含** spec 来源门（D5）、complex64 能力扩展（A3）、
执行方向确认单（目标 5/6/7）。

---

> **📍 交接入口：`dev-doc/oprunway-session-handoff-2026-08-05.md`**（实施顺序、已知坑、待办都在那）
...
- 这是正确行为（工具逻辑变了，旧收据不该继续复用），但要在
  `dev-doc/oprunway-changes-brief.md` 里写明，别让人以为复用坏了
...
> | 7 测试 | ⚠ **7.4 未满足，其余 ✅** | `test_fetch_source.py` 正/负路 + 确定性都有；本地来源另已跑通真机端到端，见 `dev-doc/oprunway-local-source-realmachine-validation.md`。**7.4 要求 adapter / driver / 三级门三处全链路都测到，实际只测到三级门** |
>
...
| **产 `vendor_build_receipt` 的外部驱动** | ⚠ **本仓不产这份收据**——全仓只有消费方（`cpp_extension_adapter.py` / `cpp_extension_driver.py` / `validate_acceptance_state.py` 三处读，加两个测试文件里的构造夹具）。要改的是**真机侧那个执行 build 并写收据的脚本**，它不在本仓。<br>**⚠ 具体文件名本方案给不出**——全仓（含 `dev-doc/`、`SKILL.md`、`agents/`）grep
...
> `dev-doc/oprunway-local-source-realmachine-validation.md`。
...
`dev-doc/oprunway-changes-brief.md` 顶部摘要与 `AGENTS.md` §4 / §9（§2.10）已同步。
