...
> 接口唯一源：[sparse-r1-registry-freeze.md](sparse-r1-registry-freeze.md)（12 字段版）。
...
| registry | skill 内版本化的有限 profile/词表数据集，装域级惯例（12 字段，见冻结文档） |
...
| 里程碑 | 内容 | 完成线 |
| --- | --- | --- |
| M0/M0.5（已完成） | 普查、投影矩阵 fixture、registry 冻结（9 字段，M7 扩到 12） | 已过 checkpoint |
...
4. 示例公共区确定性迁移（只保 FACTS 区、换公共区）——独立的最后一个机械提交；
   10 项派生物摘要不变，两个 gen_csv.py 摘要变化为预期差异（重钉协议）。
   GENERATOR_VERSION 本里程碑**不升**：它渲染进两个 verify 脚本与 README（六项
   被哈希派生物），升 2 与本完成线矛盾；升版与 v1/v2 兼容矩阵归 M3′ 裁定
   （M1′ checkpoint 结论，thread `01a06030`）。
...
1. schema v2：新键 `harness_profile`（必填）、`harness_overrides`（恰四键，冻结
   ...
   **原始字符串端到端传递，判等即文本相等**——不建规范形机制，原 M4 取消）、
   ...
   generator 版本策略随 schema v2 一并落字（自 M1′ 步骤 4 移入）：v1/v2 各自
   接受的 generator_version、最新公共区如何继续校验 v1 包、若升 2 哪些派生
   摘要属预期差异面。
...
- 文档：本文、registry-freeze（12 字段版）、census、projection-matrix、fixture
